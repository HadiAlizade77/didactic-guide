...
include_once(PATH_TO_MAIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_RECRUITER_REG_GOOGLE);
$template->set_filenames(array('registration' => 'recruiter_registration_google.htm','email'=>'recruiter_registration_template.htm','fr_email'=>'fr_recruiter_registration_template.htm'));
...
if(isset($_SESSION['access_token']))
{
 ...
 if(!tep_not_null($content['email']))
 {
  ...
  $_SESSION['google_error']='through Google unable to access your information.use normal way to register';
 ...
 {
  tep_redirect(tep_href_link(FILENAME_GOOGLE_APPLICATION,'request=jobseeker_info'));
 }
 ...
 {
  tep_redirect(tep_href_link(FILENAME_GOOGLE_APPLICATION,'request=recruiter_info'));
 }
 ...
 else if($row=getAnyTableWhereData(ADMIN_TABLE,"admin_email_address='".tep_db_input($email_address)."'","admin_id"))
 {
  ...
  $_SESSION['google_error']='This email id as admin.';
  tep_redirect(FILENAME_GOOGLE_APPLICATION1);
 }
 if(MODULE_GOOGLE_PLUGIN_RECRUITER!='enable')
 {
  $_SESSION['REDIRECT_URL']=FILENAME_RECRUITER_REGISTRATION;
  $_SESSION['google_error']='through Google recruiter registration disable by admin.use normal way to register';
  unset($_SESSION['access_token']);
  tep_redirect(FILENAME_GOOGLE_APPLICATION1);
 }
...
{
 tep_redirect(tep_href_link(FILENAME_GOOGLE_APPLICATION,'user_type=recruiter'));
}
//print_r($_POST);
//////////////////////////////
// add/edit
  ...
  case 'new':
   ...
   if(!$error)
   {
    ...
    if(!check_login('recruiter'))
    {
     ...
     tep_redirect(tep_href_link(FILENAME_GOOGLE_APPLICATION1));
    }
   }
  break;
 }
...
//////////////////////////////
{
 ...
 $registration_form=tep_draw_form('registration', FILENAME_RECRUITER_REG_GOOGLE, '', 'post', 'enctype="multipart/form-data" onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('action','new');
}
if($error)
{
