...
if(tep_not_null($article_id))
{
 if($art_row=getAnyTableWhereData(ARTICLE_TABLE,"id='".tep_db_input($article_id)."'","title,seo_name"))
 {
  ...
  $art_link=tep_href_link(get_display_link($ide,$seo_name));
...
while($category = tep_db_fetch_array($categories))
{
  ...
  <a class='list-group-item list-group-item-action' href='".tep_href_link('article_cat_'.$category['id'].'.html')."' title='".$category[TEXT_LANGUAGE.'category_name']."' class='style3'><i class='fa fa-caret-right' aria-hidden='true'></i> ".ucfirst($category[TEXT_LANGUAGE."category_name"])."</a></td></tr>
