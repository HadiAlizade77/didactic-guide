...
function getData($id)
{
    ...
    if ($result) {
        ...
        // get job title
        $title_format= HOST_NAME.$id.'/'.encode_category($result['job_title']).'.html';
        ...
        if(tep_not_null($company_logo) && is_file(PATH_TO_MAIN_PHYSICAL.PATH_TO_LOGO.$company_logo)){
            ...
            $recruiter_logo = default_logo_url($result['company_name'], false, 120);
        }
        ...
        $result['jobType'] .= tep_db_output($result['type_name']) ? tep_db_output($result['type_name']) : 'Any Type';
        $result['jobCategory'] .= $jobCategory;
        $result['logoPath'] .= $recruiter_logo;
        $result['titleLink'] .= $title_format;
        $result['apply_before'] .= date("m-d-Y", strtotime($result['expired']));
        $result['posted_on'] .= date("m-d-Y", strtotime($result['inserted']));
...
function default_logo_url($name = null, $rounded=false, $size=null)
{
