...
/**********************/
   ...
   $template->assign_block_vars('jobfairs', array( 'row_selected' => $row_selected,
    ...
    'status' => ($row_check['approved']=='Yes'?'<span class="text-success small fw-bold">APPROVED</span>':($row_check['approved']=='No'?'<span class="text-warning small fw-bold">PENDING</span>':'NOT APPLIED')),
    'noofjobs' => '<a href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBS,'jfID='.$ide).'">'.$no_of_jobs.'</a>',
    'action' => (!tep_not_null($row_check['approved'])?'<a href="' . tep_href_link(FILENAME_RECRUITER_LIST_OF_JOBFAIRS, tep_get_all_get_params(array('jobfairID','action'))).'&jobfairID='.$ide.'&action=apply_jobfair' . '" class="btn btn-text text-dark border ">'.TEXT_INFO_APPLY_NOW.'</a>':($row_check['approved']=='No'?'Applied':'Member')),//$action_image,
