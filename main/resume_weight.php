...
if(!$error)
{
 ...
 $location_weight=tep_db_output($rows['location']);
 $industry_weight=tep_db_output($rows['industry']);
 $job_type_weight=tep_db_output($rows['job_type']);
 $experience_weight=tep_db_output($rows['experience']);
 $total=$location_weight+$industry_weight+$job_type_weight+$experience_weight;
}
