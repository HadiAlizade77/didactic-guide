 ...
 switch($action1)
 {
  case 'search':
   ...
   else
   {
    ...
    $field_names   = 'a.job_id, a.application_id,j.jobseeker_first_name,j.jobseeker_last_name,jr1.experience_year, jl.jobseeker_email_address,j.jobseeker_privacy,j.jobseeker_city,jb.job_title,a.inserted, jr1.jobseeker_photo, a.resume_id';
...
//echo "ss=".get_resume_weight($row["resume_id"],$row['job_id']);die;
    ...
    'match_percentage'=>(get_resume_weight($row["resume_id"],$row['job_id'])==0 ? '0%' :tep_db_output(get_resume_weight($row["resume_id"],$row['job_id']))."%"),
		  ...
		  'applicant_pipeline'=>'<a class="btn btn-sm btn-text border text-muted me-2" href="'.tep_href_link(FILENAME_RECRUITER_APPLICATION_REPORT,'jobID='.$job_id).'#Pipeline">Applicant Pipeline</a>',
		  'contact'=>'<a class="btn btn-sm btn-text border text-muted me-2" href="'.tep_href_link(FILENAME_EMPLOYER_INTERACTION,'query_string3='.$query_string3).'">Contact</a>',
		  'view_resume'=>'<a class="btn btn-sm btn-text border text-muted me-2" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string='.$query_string).'">View Resume</a>',
...
'search_resume_form'=>tep_draw_form('search_applicant', FILENAME_RECRUITER_SEARCH_APPLICANT,'','post').tep_draw_hidden_field('action1','search'),
  'INFO_TEXT_APPLICATION1'=> tep_draw_input_field('application_id','', 'class="form-control mb-2" placeholder="Application ID"', false ),
  'INFO_TEXT_FIRST_NAME1' => tep_draw_input_field('first_name',$first_name, 'class="form-control mb-2" placeholder="First Name"',false),
  'INFO_TEXT_LAST_NAME1'  => tep_draw_input_field('last_name',$last_name, 'class="form-control mb-2" placeholder="Last Name"',false),
  'INFO_TEXT_EXPERIENCE'   => INFO_TEXT_EXPERIENCE,
  'INFO_TEXT_EXPERIENCE1'  => experience_drop_down('name="experience" class="form-select mb-2"', 'Any Experience', '', $experience),
  'INFO_TEXT_EMAIL_ADDRESS1'=> tep_draw_input_field('TNEF_email_address',$email_address, 'class="form-control mb-2" placeholder="Email Address"', false),
  'buttons'  => tep_draw_submit_button_field('','Search','class="btn btn-primary btn-block"'),
...
<li><button href="'.tep_href_link(FILENAME_RECRUITER_SEARCH_APPLICANT).'" type="button" class="ats-left-bar-btn"> > Search Applicants</button></li>
