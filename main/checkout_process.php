...
$template->set_filenames(array('email'=>'recruiter_order_template_template.htm','fr_email'=>'fr_recruiter_order_template_template.htm'));
...
if(tep_not_null(MODULE_PAYMENT_INSTALLED) && (!isset($_SESSION['payment'])))
{
 if($gift_code!='')
 {
  if($row_gift=getAnyTableWhereData(GIFT_TABLE,'certificate_number="'.tep_db_input($gift_code).'"',"*"))
  {
...
$sql_data_array = array('order_id' => $insert_id,
  ...
  if($gift_code!='')
  {
   if($row=getAnyTableWhereData(GIFT_USED_TABLE,"recruiter_id='".$_SESSION['sess_recruiterid']."' and gift_id='".$gift_id."'"))
   {
    tep_db_perform(GIFT_USED_TABLE, array('recruiter_id'=>$_SESSION['sess_recruiterid'],
                                          'gift_id'=>$gift_id,
