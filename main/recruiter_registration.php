...
$template->set_filenames(array('registration' => 'recruiter_registration.htm','email'=>'recruiter_registration_template.htm','fr_email'=>'fr_recruiter_registration_template.htm'));
   ...
   if(!$error)
   {
    ...
    else
    {
     ...
     $template->assign_vars(array(
      ...
      'logo'=>'<a href="'.tep_href_link("").'">'.tep_image(PATH_TO_IMG.DEFAULT_SITE_LOGO,tep_db_output(SITE_TITLE),'200','73','class="internal-logo"').'</a>',
...
{
 $add_save_button=tep_draw_submit_button_field('register',INFO_SIGN_UP,'class="btn btn-primary"');//tep_image_submit(PATH_TO_BUTTON.'button_submit.gif', IMAGE_INSERT);
...
if(!check_login("recruiter"))
{
 ...
 $social_button.=' <a href="'.FILENAME_FACEBOOK_APPLICATION.'?user_type=recruiter" title="Sign in with Facebook"><img class="rounded-12" src="img/facebook.png" width="24" alt="Facebook"></a>';
 if(MODULE_GOOGLE_PLUGIN=='enable' && MODULE_GOOGLE_PLUGIN_RECRUITER=='enable')
 $social_button.=' <a href="'.FILENAME_GOOGLE_APPLICATION.'?user_type=recruiter" title="Sign in with Google"><img class="rounded-12" src="img/google.png" width="24" alt="Google"></a>';
 if(MODULE_LINKEDIN_PLUGIN=='enable' && MODULE_LINKEDIN_PLUGIN_RECRUITER=='enable')
 $social_button.=' <a href="'.FILENAME_LINKEDIN_APPLICATION.'?user_type=recruiter" title="Sign in with Linkedin"><img class="rounded-12" src="img/linkedin.png" width="24" alt="Linkedin"></a>';
 if(MODULE_TWITTER_PLUGIN_RECRUITER=='enable' &&  MODULE_TWITTER_SUBMITTER_OAUTH_CONSUMER_KEY!='')
 $social_button.=' <a href="'.FILENAME_TWITTER_APPLICATION.'?user_type=recruiter" title="Sign in with Twitter"><img class="rounded-12" src="img/twitter.png" width="24" alt="Twitter"></a>';

 $social_button=trim($social_button);
 if($social_button!='')
 $social_button='<div class="social-signup"><span class="m-none">'.INFO_P_JOIN_USING.'</span> '.$social_button.'</div>';
}
...
// 'INFO_TEXT_EMAIL_ADDRESS1'=>(check_login('recruiter')?tep_draw_input_field('TREF_email_address', $TREF_email_address,'placeholder="Email address" class="form-control mb-2 required" disabled',false).tep_draw_hidden_field('TREF_email_address',$row['recruiter_email_address']):tep_draw_input_field('TREF_email_address', $TREF_email_address,'placeholder="Email address" class="form-control mb-2 required"',false)),
 ...
 'INFO_TEXT_CITY1'=>tep_draw_input_field('city', $city,'placeholder="'.INFO_P_CITY.'" class="form-control mb-2"'),
 ...
 'INFO_TEXT_TELEPHONE1'=>tep_draw_input_field('TR_telephone_number', $TR_telephone_number,'placeholder="Telephone Number" class="form-control mb-2 required"',false),
