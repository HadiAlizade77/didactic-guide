...
if($db_cover_letter_num_row > 0)
{
 ...
 while ($cover_letter = tep_db_fetch_array($db_cover_letter_query)) 
 {
  ...
  $cover_letters_array[]='<tr><td>'.tep_draw_radio_field('cover_letter', $ide, '', $ide, 'id="radio_cover_letter'.$ide.'"').'&nbsp;</td><td class="small"><label for="radio_cover_letter'.$ide.'" onMouseOver="this.style.color=\'#0000ff\'" onMouseOut="this.style.color=\'#000080\'">'.'<a href="#" onclick="popUp(\''.FILENAME_JOBSEEKER_PREVIEW_COVER_LETTER.'?cID='.$ide.'\')">'.tep_db_output($cover_letter['cover_letter_name']).'</a></label></td></tr>';
 }
...
if($db_resume_name_num_row > 0)
{
 while ($resume_name = tep_db_fetch_array($db_resume_name_query)) 
 {
  ...
  $resume_names_array[]='<tr><td>'.tep_draw_radio_field('my_resume_id', $ide, '', $ide, 'id="radio_resume_name'.$ide.'" class="form-check-input"').'&nbsp;<label class="form-check-label" for="radio_resume_name'.$ide.'" onMouseOver="this.style.color=\'#0000ff\'" onMouseOut="this.style.color=\'#000080\'"></td><td class="small"><a href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME."?resume=".$query_string1).'" target="_resume">'.tep_db_output($resume_name['resume_title']).'</a></label></td><tr>';
 }
}
else
{
...
//  'button'=>tep_image_submit(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM),
 ...
 'INFO_TEXT_DEFALUT_COVER_LETTER1'=>sprintf("<a href='".tep_href_link(FILENAME_JOBSEEKER_LIST_OF_COVER_LETTERS, 'action=add_new')."' class='text-primary'>".INFO_TEXT_ADD_NEW_COVER_LETTER."</a>"),
