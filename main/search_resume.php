...
//////////////////////////////
    ...
    $template->assign_vars(array('total'=>"<span class='badge-custom2'>$x1</span> ".$plural,'rate_mark'=>'<span class="red">**</span><span class="resume_result3">'.INFO_TEXT_RATED_ADMIN.'</span><span style="color:blue">*</span><span class="resume_result3">'.INFO_TEXT_RATED_SELF.'</span>',
					...
					'search_resume_form_keyword'=>tep_draw_input_field('keyword','','placeholder="e.g. Sales Executive" class="form-control btn-job-filter" style="width: 275px;"',false),
					'search_resume_form_location'=>LIST_TABLE(COUNTRIES_TABLE,TEXT_LANGUAGE."country_name","priority","name='country' class='form-select btn-job-filter' style='width: 175px;'","All Locations","",DEFAULT_COUNTRY_ID),
					'search_resume_form_cat'=> tep_draw_pull_down_menu('industry_sector[]', $cat_array, '', 'class="form-select btn-job-filter" style="width: 175px;"'),
					'search_resume_form_exp'=>experience_drop_down('name="experience" class="form-select btn-job-filter"', 'Experience', '', $experience),
					'search_resume_form_button'=>tep_draw_submit_button_field('login2','Search','class="btn btn-primary-sm"'),
...
array_unshift($cat_array,array("id"=>0,"text"=>"All Category"));
    ...
    $template->assign_vars(array('total'=>'No '.TABLE_HEADING_RESUME,//tep_db_output(SITE_TITLE)." ".INFO_TEXT_HAS_NOT_MATCHED,
                                 'result_class' =>'class="result_hide"',
                            					'map_view_link'=>'',
	'search_resume_form'=>tep_draw_form('search_resume', FILENAME_RECRUITER_SEARCH_RESUME,'','post').tep_draw_hidden_field('action','search'),
	'search_resume_form_keyword'=>tep_draw_input_field('keyword','','placeholder="e.g. Sales Executive" type="text" class="form-control-applicant"',false),
	'search_resume_form_location'=>LIST_TABLE(COUNTRIES_TABLE,TEXT_LANGUAGE."country_name","priority","name='country' class='form-select'","All Locations","",DEFAULT_COUNTRY_ID),
	'search_resume_form_cat'=> tep_draw_pull_down_menu('industry_sector[]', $cat_array, '', 'class="form-select"'),
   ...
   if($x > 0 && $x2 > 0)
   {
    ...
    while($row = tep_db_fetch_array($result))
    {
					...
					if(tep_not_null($row['jobseeker_photo']) && is_file(PATH_TO_MAIN_PHYSICAL_PHOTO.$row['jobseeker_photo']))
     {
      ...
      $photo = defaultProfilePhotoUrl($row['jobseeker_name'],false,100);
					}
     if(tep_not_null($row['availability_date']))
     {
      $available_status=tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_AVAILABLE, 30, 19);
     ...
     $template->assign_block_vars('search_resume_result1', array(
		...
		'working'=>(tep_not_null($experience_row['company'])?'Working in '.$experience_row['company'] :'').(tep_not_null($experience_row['job_title'])? ' As '.$experience_row['job_title'] :''),
		'check_box'=> tep_draw_checkbox_field('resume_id[]',$ide,false),
    ...
    $plural=($x2=="1")?INFO_TEXT_FEATURED_RESUME:INFO_TEXT_FEATURED_RESUMES;
				...
				($x > 0?$check_uncheck=$check_link:$check_uncheck='');
    $template->assign_vars(array('total_1'=>'<span class="badge-custom">'.$x2.'</span>'." ".$plural,'rate_mark_1'=>'<span class="red">**</span><span class="resume_result3">'.INFO_TEXT_RATED_ADMIN.'</span><span style="color:blue">*</span><span class="resume_result3">'.INFO_TEXT_RATED_SELF.'</span>',
