...
include_once(PATH_TO_MAIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_JOBSEEKER_MAILS);
$template->set_filenames(array('email'  =>'jobseeker_mails.htm','email_message'  =>'jobseeker_mails1.htm','mail_reply'  =>'jobseeker_mail_reply.htm'));
...
if(tep_not_null($action))
{
 switch($action)
 {
  case 'delete':
    ...
    {
     if($row_check=getAnyTableWhereData(APPLICANT_INTERACTION_TABLE." as ai  left outer join ".APPLICATION_TABLE . " as a on (a.id=ai.application_id)  "," a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."' and  ai.id ='".(int)$mail_id[$i]."'","ai.id,ai.sender_user"))
	 {
    ...
    tep_redirect(tep_href_link(FILENAME_JOBSEEKER_MAILS,$page_string. tep_get_all_get_params(array('lower','higher'))));
    break;
  case 'complete_delete':
    ...
    for($i=0;$i<count($mail_id);$i++)
    {
     if($row_check=getAnyTableWhereData(APPLICANT_INTERACTION_TABLE." as ai  left outer join ".APPLICATION_TABLE . " as a on (a.id=ai.application_id)  "," a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."' and  ai.id ='".(int)$mail_id[$i]."'","ai.id,ai.sender_user,ai.receiver_mail_status,ai.sender_mail_status,ai.attachment_file"))
	 {
	  ...
	  elseif($row_check['sender_user']=='jobseeker')
  ...
  case 'restore':
    ...
    for($i=0;$i<count($mail_id);$i++)
    {
     if($row_check=getAnyTableWhereData(APPLICANT_INTERACTION_TABLE." as ai  left outer join ".APPLICATION_TABLE . " as a on (a.id=ai.application_id)  "," a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."' and  ai.id ='".(int)$mail_id[$i]."'","ai.id,sender_user"))
	 {
	  if($row_check['sender_user']=='jobseeker')
  ...
  case 'marked':
    ...
    for($i=0;$i<count($mail_id);$i++)
    {
     if($row_check=getAnyTableWhereData(APPLICANT_INTERACTION_TABLE." as ai  left outer join ".APPLICATION_TABLE . " as a on (a.id=ai.application_id)  "," a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."' and  ai.id ='".(int)$mail_id[$i]."'","ai.id"))
      tep_db_query("update ".APPLICANT_INTERACTION_TABLE." set jobseeker_mark='Yes' where id='".$mail_id[$i]."'");
    }
  ...
  case 'unmarked':
    ...
    {
     if($row_check=getAnyTableWhereData(APPLICANT_INTERACTION_TABLE." as ai  left outer join ".APPLICATION_TABLE . " as a on (a.id=ai.application_id)  "," a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."' and  ai.id ='".(int)$mail_id[$i]."'","ai.id"))
      tep_db_query("update ".APPLICANT_INTERACTION_TABLE." set jobseeker_mark='No' where id='".$mail_id[$i]."'");
    }
 ...
 case 'send_mail':
    ...
    if($row_check=getAnyTableWhereData(APPLICANT_INTERACTION_TABLE." as ai  left outer join ".APPLICATION_TABLE . " as a on (a.id=ai.application_id)  "," a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."' and  ai.id ='".$mail_id."'","ai.sender_id,ai.application_id"))
	{
	 $reciver_id     =  $row_check['sender_id'];
	 $application_id =  $row_check['application_id'];
	 $sender_id      =  $_SESSION['sess_jobseekerid'];
	 ...
	 if(!$error)
	 {
      ...
      if(!$error)
	  {
	   ...
	   /////////////////////
        ...
        tep_redirect(FILENAME_JOBSEEKER_MAILS);
	   //////////////////////
	  }
	 }
	}
...
elseif(isset($_GET['query_string2'])  && $_GET['query_string2']!='')
{
 ...
 if(!$error)
 {
  if($row_check=getAnyTableWhereData(APPLICANT_INTERACTION_TABLE." as ai  left outer join ".APPLICATION_TABLE . " as a on (a.id=ai.application_id)  "," a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."' and  ai.id ='".$mail_id."'","ai.subject"))
  {
...
if(!$row_check=getAnyTableWhereData(APPLICANT_INTERACTION_TABLE." as ai  left outer join ".APPLICATION_TABLE . " as a on (a.id=ai.application_id)  "," a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."' and  ai.id ='".(int)$mail_id."'","ai.subject,ai.attachment_file,ai.message,ai.sender_user"))
{
...
if(isset($_GET['query_string2'])  && $_GET['query_string2']!='')
{
  ...
  'INFO_TEXT_MESSAGE1'=>tep_draw_textarea_field('message1', 'soft', '75%', '10', $email_text, '', true, true),
  'reply_form'        => tep_draw_form('reply_email', FILENAME_JOBSEEKER_MAILS, 'query_string2='.$query_string2, 'post', 'onsubmit="return ValidateForm(this)" enctype="multipart/form-data"').tep_draw_hidden_field('action1','send_mail'),
  'INFO_TEXT_BUTTON'  => '<a href="'.tep_href_link(FILENAME_JOBSEEKER_MAILS,"query_string1=".$query_string2).'">'.tep_draw_submit_button_field('back', IMAGE_BACK,'class="btn btn-outline-secondary"').'</a>&nbsp;&nbsp;'.tep_draw_submit_button_field('','Send','class="btn btn-primary"'),
...
elseif($mail_id>0)
{
 ...
 tep_db_query("update ".APPLICANT_INTERACTION_TABLE." set user_see='Yes' where id='".$mail_id."'");
  ...
  'INFO_TEXT_BACK'=>'<a href="'.FILENAME_JOBSEEKER_MAILS.'"><button class="btn btn-primary" >Back</button>',
  'INFO_TEXT_REPLY'=>(($row_check['sender_user']=='jobseeker')?'':'<a href="'.tep_href_link(FILENAME_JOBSEEKER_MAILS,'query_string2='.$query_string1).'" class="btn btn-primary">'.INFO_TEXT_REPLY.'</a>'),
  'RIGHT_BOX_WIDTH'   => RIGHT_BOX_WIDTH1,
  'LEFT_HTML'         => '',
		'JOB_SEARCH_LEFT'   => JOB_SEARCH_LEFT,
  'RIGHT_HTML'        => RIGHT_HTML,
...
else
{
 ...
 if($show_mail_status=='Inbox')
  $view_link1 =INFO_TEXT_VIEW_IN."<a href='#'  onclick='view_data(\"all\",\"send\")'>".INFO_TEXT_VIEW_SEND."</a> | <a href='#' class='red'  onclick='view_data(\"all\",\"inactive\")'>".INFO_TEXT_VIEW_TRASH."</a>";
 elseif($show_mail_status=='Send')
  $view_link1 =INFO_TEXT_VIEW_IN."<a href='#'  onclick='view_data(\"all\",\"active\")'>".INFO_TEXT_VIEW_INBOX."</a> | <a href='#' class='red'  onclick='view_data(\"all\",\"inactive\")'>".INFO_TEXT_VIEW_TRASH."</a>";
 else
  $view_link1 =INFO_TEXT_VIEW_IN. "<a href='#'  onclick='view_data(\"all\",\"send\")'>".INFO_TEXT_VIEW_SEND."</a> | <a href='#'   onclick='view_data(\"all\",\"active\")'>".INFO_TEXT_VIEW_INBOX."</a>";
 ...
 else if($view=='marked')
 {
  ...
  {
   $view_link =INFO_TEXT_VIEW_IN." ".$show_mail_status.": <a href='#' onclick='view_data(\"all\",\"".$mail_status."\")'>".INFO_TEXT_SHOW_ALL."</a>";
  }
  else
  {
   $view_link =INFO_TEXT_VIEW_IN." ".$show_mail_status.": <a href='#' onclick='view_data(\"all\",\"".$mail_status."\")'>".INFO_TEXT_ALL."</a>";
 ...
 $table_names=APPLICANT_INTERACTION_TABLE." as ai left join ".APPLICATION_TABLE."  as a on (a.id=ai.application_id) left join ".RECRUITER_TABLE."  as r on (ai.sender_id =r.recruiter_id and ai.sender_user='recruiter') ";
 $whereClause=" a.jobseeker_id ='".$_SESSION['sess_jobseekerid']."'";
 //print_r($_POST);
 if($mail_status=='active') //inbox
  $whereClause.=" and ai.receiver_mail_status='active' and sender_user='recruiter' ";
 elseif($mail_status=='inactive')//Trash
  $whereClause.=" and ( ( sender_user='recruiter' and ai.receiver_mail_status='inactive' )  or (ai.sender_mail_status='inactive' and sender_user='jobseeker'))";
 elseif($mail_status=='send')//send box
  $whereClause.=" and ai.sender_mail_status='active' and sender_user='jobseeker'";
 else
  $whereClause.=" and ai.receiver_mail_status='active' and sender_user='recruiter'";
 ...
 elseif($view=='marked')
  $whereClause.=" and ai.jobseeker_mark ='Yes'";

 $field_names="ai.id,a.application_id,ai.subject,ai.attachment_file,ai.inserted,ai.user_see,ai.jobseeker_mark,r.recruiter_company_name,sender_user";
 ...
 if($x > 0 && $x1 > 0)
 {
  ...
  while ($jobseeker_mail = tep_db_fetch_array($result))
  {
   ...
   'check_box' => tep_draw_checkbox_field('mail_no[]',$jobseeker_mail['id']).(($jobseeker_mail['jobseeker_mark']=='Yes')?tep_image_button('image/mark.gif',"Marked"):""),
    //'sender'        => "<a href='mailto:".tep_db_output($jobseeker_mail['email_address'])."'>".tep_db_output($jobseeker_mail['email_address'])."</a>",
    'sender'        => (($jobseeker_mail['sender_user']=='jobseeker')?'Me':"<a href='".tep_href_link(FILENAME_JOBSEEKER_MAILS,"query_string1=".$query_string1)."'>".tep_db_output($jobseeker_mail['recruiter_company_name'])."</a>"),
    'application_id'=> "<a href='".tep_href_link(FILENAME_JOBSEEKER_MAILS,"query_string1=".$query_string1)."'>".tep_db_output($jobseeker_mail['application_id'])."</a>",
    'subject'       => (($attachment_file_name!='')?' <span class="small">'.tep_image_button('img/attachment.gif',IMAGE_ATTACHMENT.' : '.tep_db_output(substr($attachment_file_name,14))).'</span>':'')." <a href='".tep_href_link(FILENAME_JOBSEEKER_MAILS,"query_string1=".$query_string1)."'>".tep_db_output($jobseeker_mail['subject'])."</a>",
 ...
 $template->assign_vars(array(
  ...
  'form'=>tep_draw_form('page', FILENAME_JOBSEEKER_MAILS,'','post', 'onsubmit="return ValidateForm(this)"'),
  'view_link'   => $view_link,
