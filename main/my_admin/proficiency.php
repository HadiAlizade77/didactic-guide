 ...
 switch ($action) 
  ...
  case 'save':
   $proficiency=tep_db_prepare_input($_POST['TR_Proficiency']);
 		$fr_proficiency=tep_db_prepare_input($_POST['TR_fr_Proficiency']);
   $priority = tep_db_prepare_input($_POST['IN_priority']);
   $sql_data_array['language_proficiency'] = $proficiency;
			$sql_data_array['fr_language_proficiency'] = $fr_proficiency;
   //$sql_data_array['priority'] = $priority;
			...
			if($action=='insert')
			{
				...
				elseif($row_chek=getAnyTableWhereData(LANGUAGE_PROFICIENCY_TABLE," fr_language_proficiency='".tep_db_input($fr_proficiency)."'",'id'))
				{
    ...
    $id=(int)$_GET['id'];
				...
				else if($row_chek=getAnyTableWhereData(LANGUAGE_PROFICIENCY_TABLE,"fr_language_proficiency='".tep_db_input($fr_proficiency)."' and id!='$id'",'id'))
				{
					$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
					$action='edit';
				}
...
///////////// Middle Values 
$proficiency_query_raw="select id, language_proficiency,fr_language_proficiency from " . LANGUAGE_PROFICIENCY_TABLE ." order by language_proficiency";
...
if(tep_db_num_rows($proficiency_query) > 0)
{
 ...
 while ($row_proficiency = tep_db_fetch_array($proficiency_query)) 
 {
  ...
  $template->assign_block_vars('proficiency', array( 'row_selected' => $row_selected,
   ...
   'fr_name' => tep_db_output($row_proficiency['fr_language_proficiency']),
...
switch ($action) 
{
 ...
 case 'save':
  ...
  '.tep_draw_input_field('TR_fr_Proficiency', $_POST['TR_fr_Proficiency'], 'class="form-control form-control-sm"' ).'
  </div>
 ...
 case 'edit':
  ...
  <div class="form-group">
  <label>'.TEXT_INFO_FR_PROFICIENCY_NAME.'</label>
  '.tep_draw_input_field('TR_fr_Proficiency', $cInfo->fr_language_proficiency,'class="form-control form-control-sm"').'
  </div>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_PROFICIENCY, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
