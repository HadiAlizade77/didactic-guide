...
if ($action!="") 
{
 switch ($action) 
  ...
  case 'save':
   ...
   $fr_education_level_name=tep_db_prepare_input($_POST['TR_fr_education_level_name']);
   $priority = tep_db_prepare_input($_POST['IN_priority']);
   $sql_data_array['education_level_name'] = $education_level_name;
   $sql_data_array['fr_education_level_name'] = $fr_education_level_name;
   $sql_data_array['priority'] = $priority;
			...
			if($action=='insert')
			{
				...
				else if($row_chek=getAnyTableWhereData(EDUCATION_LEVEL_TABLE,"fr_education_level_name='".tep_db_input($fr_education_level_name)."'",'id'))
				{
    ...
    $id=(int)$_GET['id'];
				...
				else if($row_chek=getAnyTableWhereData(EDUCATION_LEVEL_TABLE,"fr_education_level_name='".tep_db_input($fr_education_level_name)."' and id!='$id'",'id'))
				{
					$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
					$action='edit';
				}
...
///////////// Middle Values 
$education_level_query_raw="select id, education_level_name,fr_education_level_name,priority from " . EDUCATION_LEVEL_TABLE ." order by priority";
...
if(tep_db_num_rows($education_level_query) > 0)
{
 ...
 while ($education_level = tep_db_fetch_array($education_level_query)) 
 {
  ...
  $template->assign_block_vars('education_level', array( 'row_selected' => $row_selected,
   ...
   'fr_name' => tep_db_output($education_level['fr_education_level_name']),
...
switch ($action) 
{
 ...
 case 'save':
  ...
  '.tep_draw_input_field('TR_fr_education_level_name', $_POST['TR_fr_education_level_name'], 'class="form-control form-control-sm"' ).'
  </div>
  <div class="form-group">
 ...
 case 'edit':
  ...
  <div class="form-group">
  <label>'.TEXT_INFO_FR_EDUCATION_LEVEL_NAME.'</label>
  '.tep_draw_input_field('TR_fr_education_level_name', $cInfo->fr_education_level_name, 'class="form-control form-control-sm"' ).'
  </div>
  <div class="form-group">
  <label>'.TEXT_INFO_EDUCATION_LEVEL_PRIORITY.'</label>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_EDUCATION_LEVEL, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
