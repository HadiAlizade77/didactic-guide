...
if (tep_not_null($action)) 
{
 switch ($action) 
 {
  case 'insert':
   ...
   $fr_zone_name = tep_db_prepare_input($_POST['fr_zone_name']);
   tep_db_query("insert into " . ZONES_TABLE . " (zone_country_id, zone_code, zone_name,fr_zone_name) values ('" . (int)$zone_country_id . "', '" . tep_db_input($zone_code) . "', '" . tep_db_input($zone_name) . "','" . tep_db_input($fr_zone_name) . "')");
  ...
  case 'save':
   ...
   tep_db_query("update " . ZONES_TABLE . " set zone_country_id = '" . (int)$zone_country_id . "', zone_code = '" . tep_db_input($zone_code) . "', zone_name = '" . tep_db_input($zone_name) . "' , fr_zone_name = '" . tep_db_input($fr_zone_name) . "' where zone_id = '" . (int)$zone_id . "'");
...
///////////// Middle Values 
$zones_query_raw = "select z.zone_id, c.id, c.country_name, z.zone_name , z.fr_zone_name , z.zone_code, z.zone_country_id from " . ZONES_TABLE . " z, " . COUNTRIES_TABLE . " c where z.zone_country_id = c.id order by c.country_name, z.zone_name";
...
if(tep_db_num_rows($zones_query) > 0)
{
 ...
 while ($zones = tep_db_fetch_array($zones_query)) 
 {
  ...
  $template->assign_block_vars('zones', array( 'row_selected' => $row_selected,
   ...
   'fr_zone_name' => tep_db_output($zones['fr_zone_name']),
...
switch ($action) 
{
 case 'new':
  ...
  // $contents[] = array('text' => '<br>' . TEXT_INFO_FR_ZONES_NAME . '<br>' . tep_draw_input_field('fr_zone_name'));
  ...
  $contents[] = array('align' => 'left', 'text' => '<div class="py-2">
    ...
    '.tep_draw_input_field('fr_zone_name','','class="form-control form-control-sm"').'
    </div>
    <div class="form-group">
 ...
 case 'edit':
  ...
  // $contents[] = array('text' => '<br>' . TEXT_INFO_FR_ZONES_NAME . '<br>' . tep_draw_input_field('fr_zone_name', $zInfo->fr_zone_name));
  ...
  <div class="form-group">
  <label>'.TEXT_INFO_FR_ZONES_NAME.'</label>
  '.tep_draw_input_field('fr_zone_name',$zInfo->fr_zone_name,'class="form-control form-control-sm"').'
  </div>
  <div class="form-group">
  <label>'.TEXT_INFO_ZONES_CODE.'</label>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_ZONE, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
