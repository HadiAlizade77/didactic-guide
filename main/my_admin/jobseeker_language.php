...
if ($action!="") 
{
 switch ($action) 
  ...
  case 'save':
   ...
   $fr_language_name=tep_db_prepare_input($_POST['TR_fr_language_name']);
   $priority = tep_db_prepare_input($_POST['IN_priority']);
   $sql_data_array['name'] = $language_name;
			$sql_data_array['fr_name'] = $fr_language_name;
   $sql_data_array['priority'] = $priority;
			...
			if($action=='insert')
			{
				...
				else if($row_chek=getAnyTableWhereData(JOBSEEKER_LANGUAGE_TABLE," fr_name='".tep_db_input($fr_language_name)."'",'languages_id'))
				{
    ...
    $languages_id=(int)$_GET['languages_id'];
				...
				else if($row_chek=getAnyTableWhereData(JOBSEEKER_LANGUAGE_TABLE," fr_name='".tep_db_input($fr_language_name)."' and languages_id!='$languages_id'",'languages_id'))
				{
					$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
					$action='edit';
				}
...
///////////// Middle Values 
$language_level_query_raw="select languages_id, name,fr_name,priority from " . JOBSEEKER_LANGUAGE_TABLE ." order by priority";
...
if(tep_db_num_rows($language_level_query) > 0)
{
 ...
 while ($language_level = tep_db_fetch_array($language_level_query)) 
 {
  ...
  $template->assign_block_vars('language_level', array( 'row_selected' => $row_selected,
   ...
   'fr_name' => tep_db_output($language_level['fr_name']),
...
switch ($action) 
{
 ...
 case 'save':
  ...
  $contents = array('form' => tep_draw_form('language_level', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'action=insert','post',' onsubmit="return ValidateForm(this)"'));
    ...
    '.tep_draw_input_field('TR_fr_language_name', $_POST['TR_fr_language_name'], 'class="form-control form-control-sm"' ).'
    </div>
    <div class="form-group">
 ...
 case 'edit':
  ...
  <div class="form-group">
  <label>'.TEXT_INFO_FR_LANGUAGE_NAME.'</label>
  '.tep_draw_input_field('TR_fr_language_name', $cInfo->fr_name, 'class="form-control form-control-sm"' ).'
  </div>
  <div class="form-group">
  <label>'.TEXT_INFO_LANGUAGE_PRIORITY.'</label>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_LANGUAGE, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
