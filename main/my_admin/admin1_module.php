 ...
 switch ($action)
  ...
  // $sql_data_array['configuration_title'] = $configuration_title;
   ...
   if($configuration_name =='DEFAULT_SITE_LOGO')
   {
    ...
    {
     if($obj_logo = new upload('site_logo',PATH_TO_MAIN_PHYSICAL.PATH_TO_IMG,'644',array('gif','jpg','png')))
					{
...
/////////////////////////////////////////////////////////////////
 ...
 default:
  if (isset($cInfo) && is_object($cInfo))
   ...
   $contents[] = array('text' => '<p>'.TEXT_INFO_DATE_ADDED. date("m-d-Y", strtotime($cInfo->inserted)).'</p>');
   if (tep_not_null($cInfo->updated))
			 $contents[] = array('text' => '<p>' .TEXT_INFO_DATE_UPDATED. date("m-d-Y", strtotime($cInfo->updated)).'</p>');
  }
  break;
}
