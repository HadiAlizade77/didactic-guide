 ...
 switch ($action) 
  ...
  case 'save':
   $career_level_name=tep_db_prepare_input($_POST['TR_career_level_name']);
 		$fr_career_level_name=tep_db_prepare_input($_POST['TR_fr_career_level_name']);
   $priority = tep_db_prepare_input($_POST['IN_priority']);
   $sql_data_array['career_level_name'] = $career_level_name;
			$sql_data_array['fr_career_level_name'] = $fr_career_level_name;
   $sql_data_array['priority'] = $priority;
			...
			if($action=='insert')
			{
				...
				else if($row_chek=getAnyTableWhereData(CAREER_LEVEL_TABLE," fr_career_level_name='".tep_db_input($fr_career_level_name)."'",'id'))
				{
    ...
    $id=(int)$_GET['id'];
				...
				else if($row_chek=getAnyTableWhereData(CAREER_LEVEL_TABLE," fr_career_level_name='".tep_db_input($fr_career_level_name)."' and id!='$id'",'id'))
				{
					$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
					$action='edit';
				}
...
///////////// Middle Values 
$career_level_query_raw="select id, career_level_name,fr_career_level_name,priority from " . CAREER_LEVEL_TABLE ." order by priority";
...
if(tep_db_num_rows($career_level_query) > 0)
{
 ...
 while ($career_level = tep_db_fetch_array($career_level_query)) 
 {
  ...
  $template->assign_block_vars('career_level', array( 'row_selected' => $row_selected,
   ...
   'fr_name' => tep_db_output($career_level['fr_career_level_name']),
...
switch ($action) 
{
 ...
 case 'save':
  ...
  $contents = array('form' => tep_draw_form('career_level', PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_CAREER_LEVEL, 'action=insert','post',' onsubmit="return ValidateForm(this)"'));
    ...
    '.tep_draw_input_field('TR_fr_career_level_name', $_POST['TR_fr_career_level_name'], 'class="form-control form-control-sm"' ).'
    </div>
    <div class="form-group">
 ...
 case 'edit':
  ...
  <div class="form-group">
  <label>'.TEXT_INFO_FR_CAREER_LEVEL_NAME.'</label>
  '.tep_draw_input_field('TR_fr_career_level_name', $cInfo->fr_career_level_name, 'class="form-control form-control-sm"' ).'
  </div>
  <div class="form-group">
  <label>'.TEXT_INFO_CAREER_LEVEL_PRIORITY.'</label>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_CAREER_LEVEL, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
