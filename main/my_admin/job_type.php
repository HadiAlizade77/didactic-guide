 ...
 switch ($action) 
  ...
  case 'save':
   $type_name=tep_db_prepare_input($_POST['TR_job_type_name']);
 		$fr_type_name=tep_db_prepare_input($_POST['TR_fr_job_type_name']);
   $priority = tep_db_prepare_input($_POST['IN_priority']);
   $sql_data_array['type_name'] = $type_name;
			$sql_data_array['fr_type_name'] = $fr_type_name;
   $sql_data_array['priority'] = $priority;
			...
			if($action=='insert')
			{
				...
				elseif($row_chek=getAnyTableWhereData(JOB_TYPE_TABLE,"fr_type_name='".tep_db_input($fr_type_name)."'",'id'))
				{
    ...
    $id=(int)$_GET['id'];
				...
				else if($row_chek=getAnyTableWhereData(JOB_TYPE_TABLE,"fr_type_name='".tep_db_input($fr_type_name)."' and id!='$id'",'id'))
				{
					$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
					$action='edit';
				}
...
///////////// Middle Values 
$job_type_query_raw="select id, type_name,fr_type_name,priority from " . JOB_TYPE_TABLE ." order by type_name";
...
if(tep_db_num_rows($job_type_query) > 0)
{
 ...
 while ($job_type = tep_db_fetch_array($job_type_query)) 
 {
  ...
  $template->assign_block_vars('job_type', array( 'row_selected' => $row_selected,
   ...
   'fr_name' => tep_db_output($job_type['fr_type_name']),
...
switch ($action) 
{
 ...
 case 'save':
    ...
    '.tep_draw_input_field('TR_fr_job_type_name', $_POST['TR_fr_job_type_name'], 'class="form-control form-control-sm"' ).'
    </div>
    <div class="form-group">
 ...
 case 'edit':
  ...
  <div class="form-group">
  <label>'.TEXT_INFO_FR_JOB_TYPE_NAME.'</label>
  '.tep_draw_input_field('TR_fr_job_type_name', $cInfo->fr_type_name, 'class="form-control form-control-sm"' ).'
  </div>
  <div class="form-group">
  <label>'.TEXT_INFO_JOB_TYPE_PRIORITY.'</label>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOB_TYPE, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
