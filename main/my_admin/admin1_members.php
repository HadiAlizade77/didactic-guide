...
if(tep_not_null($action)) 
{
 switch ($action) 
 {
  case 'member_new':
   ...
   if($row=getAnyTableWhereData(ADMIN_TABLE,"admin_name='".tep_db_input($admin_name)."' ","admin_id"))
   {
...
else
{
 $template->assign_vars(array(
  ...
  <a class="btn btn-primary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_MEMBERS, 'page=' . $_GET['page'] . '&mID=' . $mInfo->admin_id . '&action=new_member') . '"><i class="fa fa-plus" aria-hidden="true"></i> ' . IMAGE_NEW_MEMBER . '</a>',
  'RIGHT_BOX_WIDTH'=>$RIGHT_BOX_WIDTH,
  'ADMIN_RIGHT_HTML'=>$ADMIN_RIGHT_HTML,
  'update_message'=>$messageStack->output()));
