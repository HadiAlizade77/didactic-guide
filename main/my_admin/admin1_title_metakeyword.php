...
if(tep_not_null($file_id))
{
 ...
 else
 {
  ...
  $fr_title=stripslashes($row['fr_title']);
  $fr_meta_keyword=stripslashes($row['fr_meta_keyword']);
 }
}
if ($action!="") 
{
 switch ($action) 
  ...
  $middle_string1.="<td class='mt-3'>".tep_draw_input_field('fr_title', $fr_title, 'size="45" class="form-control form-control-sm"', false).'</td></tr>';
  ...
  $middle_string2.='<tr><td colspan="2">'.tep_draw_textarea_field('fr_meta_keyword', 'soft', '90%', '5', $fr_meta_keyword, 'class="form-control form-control-sm mt-2"', true, true).'</td></tr>';
  ...
  if($action=='insert')
  {
   ...
   $fr_title=tep_db_prepare_input($_POST['fr_title']);
   $fr_meta_keyword=$_POST['fr_meta_keyword'];
   if(isset($_POST['title']))
    $sql_data_array['title']=$title;
   if(isset($_POST['meta_keyword']))
    $sql_data_array['meta_keyword']=$meta_keyword;
   if(isset($_POST['fr_title']))
    $sql_data_array['fr_title']=$fr_title;
   if(isset($_POST['fr_meta_keyword']))
    $sql_data_array['fr_meta_keyword']=$fr_meta_keyword;
    tep_db_perform(TITLE_KEYWORDMETATYPE_TABLE, $sql_data_array,'update',"id='".tep_db_input($file_id)."'");
   tep_redirect(FILENAME_ADMIN1_TITLE_METAKEYWORDS);
  }
  break;
...
//  '.tep_image_button(PATH_TO_BUTTON.'button_new.gif',IMAGE_NEW).'</a>&nbsp;&nbsp;',
 ...
 <i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
