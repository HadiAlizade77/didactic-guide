...
if ($action!="")
{
 switch ($action)
  ...
  //echo $duration_unlimited;
   ...
   $filetype_array=array('jpg','jpeg','gif','bmp');
   $errmsg="Only jpg,jpeg,gif,bmp File Types are Allowed to upload.";
   if($action=='insert_banner')
   {
    ...
    elseif($banner_type=='image')
    {
					if(!$error)
					{
						if(tep_not_null($_FILES['banner']['name']))
							{
								if($obj_logo = new upload('banner',PATH_TO_MAIN_PHYSICAL_BANNER,'644',array('gif','jpg','png')))
								{
...
if(tep_db_num_rows($banner_query) > 0)
{
 ...
 while ($banner = tep_db_fetch_array($banner_query))
 {
  ...
  $template->assign_block_vars('banner', array( 'row_selected' => $row_selected,
   ...
   'location'=>$location,
   'bdate'=>date("m-d-Y", strtotime($banner['banner_date'])),
   'view'=> $view  ,
  // 'action'=>$action_image,
   ));
 }
...
$template->assign_vars(array(
 ...
 'button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_BANNER_UPLOAD, 'action=new_banner') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
