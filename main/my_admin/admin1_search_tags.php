...
if(tep_db_num_rows($search_tag_query) > 0)
{
 ...
 while ($search_tag = tep_db_fetch_array($search_tag_query)) 
 {
  ...
  {
   $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif',STATUS_TAG_INACTIVE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS, tep_get_all_get_params(array('id','action','selected_box'))).'&id='.$search_tag['id'].'&action=tag_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_TAG_ACTIVATE, 30, 20) . '</a>';
  }
...
$template->assign_vars(array(
 ...
 <a class="btn btn-primary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SEARCH_TAGS, 'page=' . $_GET['page'] .'&action=new') . '"> <i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
