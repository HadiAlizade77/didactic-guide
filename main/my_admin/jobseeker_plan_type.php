...
if ($action!="") 
{
 switch ($action) 
  ...
  case 'save':
   ...
   $fr_plan_type_name=tep_db_prepare_input($_POST['TR_fr_plan_type_name']);
   ...
   $sql_data_array['fr_plan_type_name'] = $fr_plan_type_name;
   ...
   if($action=='insert')
	{
	 if($row_chek=getAnyTableWhereData(JOBSEEKER_PLAN_TYPE_TABLE,"plan_type_name='".tep_db_input($plan_type_name)."'",'id'))
	  ...
	  else if($row_chek=getAnyTableWhereData(JOBSEEKER_PLAN_TYPE_TABLE,"fr_plan_type_name='".tep_db_input($fr_plan_type_name)."'",'id'))
	  {
	...
	else
	 {
	  ...
	  if($row_chek=getAnyTableWhereData(JOBSEEKER_PLAN_TYPE_TABLE,"fr_plan_type_name='".tep_db_input($fr_plan_type_name)."' and id!='$id'",'id'))
	   {
		$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
		$action='edit';
	   }
...
if(tep_db_num_rows($plan_type_query) > 0)
{
 ...
 while ($plan_type = tep_db_fetch_array($plan_type_query)) 
 {
  ...
  $template->assign_block_vars('plan_type', array( 'row_selected' => $row_selected,
   ...
   'fr_name' => tep_db_output($plan_type['fr_plan_type_name']),
...
switch ($action) 
{
 ...
 case 'save':
  ...
  // $contents[] = array('text' => '<br>'.TEXT_INFO_FR_PLAN_TYPE_NAME.'<br>'.tep_draw_input_field('TR_fr_plan_type_name','', '' ));
  ...
  '.tep_draw_input_field('TR_fr_plan_type_name','', 'class="form-control form-control-sm"' ).'
  </div>
  <div class="form-group">
 ...
 case 'edit':
  ...
  // $contents[] = array('text' => '<br>'.TEXT_INFO_FR_PLAN_TYPE_NAME.'<br>'.tep_draw_input_field('TR_fr_plan_type_name', $cInfo->fr_plan_type_name, '' ));
  ...
  <div class="form-group">
  <label>'.TEXT_INFO_FR_PLAN_TYPE_NAME.'</label>
  '.tep_draw_input_field('TR_fr_plan_type_name', $cInfo->fr_plan_type_name, 'class="form-control form-control-sm"' ).'
  </div>
  <div class="form-group">
  <label>'.TEXT_INFO_PLAN_TYPE_TIME.'</label>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_JOBSEEKER_PLAN_TYPE, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> 
 '.IMAGE_NEW.'</a>',
