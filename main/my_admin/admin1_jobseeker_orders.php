...
if (($action == 'edit') && ($order_exists == true))
{
 ...
 {
  $order_total_text.='
		<dt class="col-5">'.$order->totals[$i]['title'].'</dt>
		<dd class="col-7">'.$order->totals[$i]['text'].'</dd>
		'."\n";
 }
	$template->assign_vars(array(
		'HEADING_TITLE'=>'<a class="btn btn-secondary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKER_ORDERS, tep_get_all_get_params(array('action','selected_box'))) . '">' . IMAGE_BACK . '</a>',
		...
		'TABLE_HEADING_DATE_ADDED1'=>date("m-d-Y", strtotime($order->info['date_purchased'])),
		...
		'TABLE_HEADING_COMMENTS2'=>tep_draw_textarea_field('comments', 'soft', '50', '5',$admin_comments,'class="form-control form-control-sm"'),
		...
		'ENTRY_STATUS1'=>tep_draw_pull_down_menu('status', $orders_statuses, $orders_status,'class="form-control form-control-sm"'),
...
else
{
 ...
 $alternate=1;
	while ($orders = tep_db_fetch_array($orders_query))
	{
		...
		{
			$row_selected=' id="defaultSelected" class="table-secondary dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKER_ORDERS, tep_get_all_get_params(array('oID', 'action','selected_box')) . 'oID=' . $oInfo->orders_id . '&action=edit') . '\'"';
  ...
  $template->assign_block_vars('order', array( 'row_selected' => $row_selected,
   ...
   'name' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKER_ORDERS, tep_get_all_get_params(array('oID', 'action','selected_box')) . 'oID=' . $orders['orders_id'] . '&action=edit') . '">' . tep_db_output($orders['jobseeker_name']) . '</a>',
   'view' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKER_ORDERS, tep_get_all_get_params(array('oID', 'action','selected_box')) . 'oID=' . $orders['orders_id'] . '&action=edit') . '">' . tep_image(PATH_TO_IMAGE . 'preview.gif', 'Edit') . '</a>',
   'order_total' => strip_tags($orders['order_total']),
   'date_purchased' => date("m-d-Y", strtotime($orders['date_purchased'])),
...
switch ($action)
{
	case 'delete':
		...
		// ' . tep_image_submit(PATH_TO_BUTTON.'button_delete.gif', IMAGE_DELETE) . '
		// <a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKER_ORDERS, tep_get_all_get_params(array('oID', 'action','selected_box')) . 'oID=' . $oInfo->orders_id) . '">
		...
		$contents[] = array('align' => 'left', 'text' => '<div class="py-2">
			...
			. tep_button('Delete','class="btn btn-primary mt-2"') . '
			<a class="btn btn-secondary mt-2" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKER_ORDERS,
				tep_get_all_get_params(array('oID', 'action','selected_box')) . 'oID=' . $oInfo->orders_id) . '">Cancel</a>
			</div>
		');
	break;
	default:
		if (isset($oInfo) && is_object($oInfo))
		{
			...
			$heading[] = array('text' => '<div class="py-2">
											[' . $oInfo->orders_id . ']&nbsp;&nbsp;' . date("m-d-Y", strtotime($oInfo->date_purchased)) . '
										</div>
						');

			$contents[] = array('align' => 'left', 'text' => '<div class="py-2">
									<a class="btn btn-primary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKER_ORDERS,
									tep_get_all_get_params(array('oID', 'action','selected_box')) . 'oID=' . $oInfo->orders_id . '&action=edit') . '">
									Edit
									</a>
									<a class="btn btn-secondary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKER_ORDERS,
			...
			$contents[] = array('text' => '<div class="py-2">' . TEXT_DATE_ORDER_CREATED . ' ' . date("m-d-Y", strtotime($oInfo->date_purchased))
			.'</div>'
			);
			if (tep_not_null($oInfo->last_modified)){
				$contents[] = array('text' => '<div class="py-2">'. TEXT_DATE_ORDER_LAST_MODIFIED . ' ' . date("m-d-Y", strtotime($oInfo->last_modified)).'</div>');
