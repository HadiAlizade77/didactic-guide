...
if($db_jobseeker_num_row > 0)
{
 ...
 while ($jobseeker = tep_db_fetch_array($db_jobseeker_query))
 {
  if($action1!='delete')
  {
   ...
   {
    $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOBSEEKER_INACTIVE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOBSEEKER_ACTIVATE, 30, 20) . '</a>';
   }
   if ($jobseeker['jobseeker_featured'] == 'Yes')
   {
    $featured_status='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_not_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_JOBSEEKER_NOT_FEATURED, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_JOBSEEKER_FEATURED, 30, 20);
   }
   else
   {
    $featured_status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOBSEEKER_NOT_FEATURE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOBSEEKER_FEATURED, 30, 20) . '</a>';
   }
  }
  else
  {
...
if($action1=='delete')
{
 $check_link='<br>'.TEXT_DELETE_WARNING.'<br><br><a class="btn-text text-danger" href="#" onclick="DeleteSelected(\'confirm_bulk_delete\')">'.tep_button('Confirm','class="btn btn-primary"').'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_JOBSEEKERS).'">'.tep_button('Cancel','class="btn btn-primary"').'</a>';
...
else
{
 ...
 <i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> New</a>',
