...
if($action!="") 
{
 switch ($action) 
  ...
  case 'save1':
   ...
   $fr_category_name=tep_db_prepare_input($_POST['TR_fr_forum_category_name']);
   ...
   if($action=='insert')
	{
	 ...
	 elseif($row_chek=getAnyTableWhereData(FORUM_CATEGORIES_TABLE,"fr_category_name='".tep_db_input($fr_category_name)."'",'id'))
	  {
	   $messageStack->add(MESSAGE_NAME_ERROR, 'error');
	  }
	 else
	...
	else if($action=='save')
	 {
	  ...
	  else if($row_chek=getAnyTableWhereData(FORUM_CATEGORIES_TABLE,"fr_category_name='".tep_db_input($fr_category_name)."' and id!='$id'",'id'))
	   {
		$messageStack->add(MESSAGE_NAME_ERROR, 'error');
		$action='edit';
	   }
...
///////////// Middle Values 
$forum_category_query_raw="select id, category_name,fr_category_name from " . FORUM_CATEGORIES_TABLE ." order by category_name asc";
 ...
 while ($forum_category = tep_db_fetch_array($forum_category_query)) 
 {
  ...
  { 
   ...
   $category_name=$forum_category['category_name'];
  		///french//
   $fr_category_name=$forum_category['fr_category_name'];
...
switch ($action) 
{
 ...
 case 'insert':
   ...
   $contents  = array('form' => tep_draw_form('forum_category', PATH_TO_ADMIN.FILENAME_ADMIN1_FORUM_CATEGORIES, 'action=insert','post',' onsubmit="return ValidateForm(this)"'));
	...
	// $contents[] = array('text' => '<br>'.TEXT_INFO_FR_FORUM_CATEGORY_NAME.'<br>'.tep_draw_input_field('TR_fr_forum_category_name', ($action=='insert'?$_POST['TR_fr_forum_category_name']:''), '' ));
   ...
   <div class="mb-1">'.TEXT_INFO_NEW_INTRO.'</div> 
      ...
      '.tep_draw_input_field('TR_fr_forum_category_name', ($action=='insert'?$_POST['TR_fr_forum_category_name']:''), 'class="form-control form-control-sm"' ).'
      </div>
...
//   $contents[] = array('text' => '<br>'.TEXT_INFO_FR_FORUM_CATEGORY_NAME.'<br>'.tep_draw_input_field('TR_fr_forum_category_name', ($action=='save'?$_POST['TR_fr_forum_category_name']:$cInfo->fr_category_name), '' ));
//   $contents[] = array('align' => 'left', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FORUM_CATEGORIES).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
  ...
  <div class="mb-1">'.TEXT_INFO_EDIT_INTRO.'</div> 
     ...
     <div class="form-group">
     <label>'.TEXT_INFO_FR_FORUM_CATEGORY_NAME.'</label>
     '.tep_draw_input_field('TR_fr_forum_category_name', ($action=='save'?$_POST['TR_fr_forum_category_name']:$cInfo->fr_category_name), 'class="form-control form-control-sm"' ).'
     </div>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_FORUM_CATEGORIES, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus" aria-hidden="true"></i> '.IMAGE_NEW1.'</a>',
