...
include_once(PATH_TO_MAIN_ADMIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_ADMIN1_PAID_JOBSEEKERS);
$template->set_filenames(array('jobseekers' => 'admin1_list_of_paid_jobseekers.htm'));
...
if(isset($_GET['jID']))
{
 ...
 if(!$row_check_jobseeker=getAnyTableWhereData(JOBSEEKER_TABLE,"jobseeker_id='".$jobseeker_id."'","jobseeker_id"))
 {
  ...
  tep_redirect(tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS));
 }
}
$whereClause='';
...
if(tep_not_null($action))
{
 switch ($action)
 {
  ...
  case 'jobseeker_inactive':
   ...
   tep_redirect(tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS,tep_get_all_get_params(array('action','selected_box'))));
   break;
...
if(tep_not_null($search_status1))
{
 ...
 {
  $db_jobseeker_query_raw = "select j.jobseeker_id, concat(j.jobseeker_first_name,' ',j.jobseeker_last_name) as jobseeker_name, j.jobseeker_featured, jl.updated, jl.jobseeker_email_address, jl.jobseeker_status, jl.ip_address, jl.last_login_time, jl.number_of_logon  from " . JOBSEEKER_LOGIN_TABLE . " as jl, " . JOBSEEKER_TABLE . " as j where jl.jobseeker_id=j.jobseeker_id and jobseeker_status='Yes' and j.jobseeker_id IN(select jobseeker_id from ".JOBSEEKER_ORDER_TABLE.") $whereClause order by ".$order_by_clause;
 }
 elseif($search_status1=='inactive')
 {
  $db_jobseeker_query_raw = "select j.jobseeker_id, concat(j.jobseeker_first_name,' ',j.jobseeker_last_name) as jobseeker_name, j.jobseeker_featured, jl.updated, jl.jobseeker_email_address, jl.jobseeker_status, jl.ip_address, jl.last_login_time, jl.number_of_logon  from " . JOBSEEKER_LOGIN_TABLE . " as jl, " . JOBSEEKER_TABLE . " as j where jl.jobseeker_id=j.jobseeker_id and jobseeker_status='No' and j.jobseeker_id IN(select jobseeker_id from ".JOBSEEKER_ORDER_TABLE.") $whereClause order by ".$order_by_clause;
 }
}
else
$db_jobseeker_query_raw = "select j.jobseeker_id, concat(j.jobseeker_first_name,' ',j.jobseeker_last_name) as jobseeker_name, j.jobseeker_featured, jl.updated, jl.jobseeker_email_address, jl.jobseeker_status, jl.ip_address, jl.last_login_time, jl.number_of_logon  from " . JOBSEEKER_LOGIN_TABLE . " as jl, " . JOBSEEKER_TABLE . " as j where jl.jobseeker_id=j.jobseeker_id and j.jobseeker_id IN(select jobseeker_id from ".JOBSEEKER_ORDER_TABLE.") $whereClause order by ".$order_by_clause;
...
if($db_jobseeker_num_row > 0)
{
 ...
 while ($jobseeker = tep_db_fetch_array($db_jobseeker_query))
 {
  if($action1!='delete')
  {
   ...
   $job_title='';
   if($check=getAnyTableWhereData(JOBSEEKER_RESUME1_TABLE ,$wclause,"target_job_titles, resume_id"))
   {
   ...
   {
    //$row_selected=' class="dataTableRow'.($alternate%2==1?'1':'2').'" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID='.$jobseeker['jobseeker_id'] . '\'"';
    $row_selected=' class="dataTableRow'.($alternate%2==1?'1':'2').'" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"';
    $action_image='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID='.$jobseeker['jobseeker_id'] . '">'.tep_image(PATH_TO_IMAGE.'icon_info.gif',IMAGE_INFO).'</a>';
   }
   if ($jobseeker['jobseeker_status'] == 'Yes')
   {
    $status='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_inactive' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_JOBSEEKER_INACTIVATE, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_JOBSEEKER_ACTIVE, 30, 20);
   }
   else
   {
    $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOBSEEKER_INACTIVE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOBSEEKER_ACTIVATE, 30, 20) . '</a>';
   }
   if ($jobseeker['jobseeker_featured'] == 'Yes')
   {
    $featured_status='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_not_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_JOBSEEKER_NOT_FEATURED, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_JOBSEEKER_FEATURED, 30, 20);
   }
   else
   {
    $featured_status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOBSEEKER_NOT_FEATURE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','action','selected_box'))).'&jID=' . $jobseeker['jobseeker_id'] . '&action=jobseeker_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOBSEEKER_FEATURED, 30, 20) . '</a>';
   }
  }
  else
  {
...
switch ($action)
{
 case 'delete':
  ...
  // $contents[] = array('text' => '<br><b>' . $rInfo->jobseeker_name . '</b>');
  $contents[] = array('align' => 'left', 'text' => '<div class="py-2">
          <a class="btn btn-primary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('page','action','selected_box'))).'&action=confirm_delete' . '">
          Confirm</a>
          <a class="btn btn-secondary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('action','selected_box'))) . '">
          Cancel</a>
          </div>');
 ...
 {
  $heading[] = array('text' => '<div class="list-group"><div class="font-weight-bold">'.$rInfo->jobseeker_name.'</div></div>');
  // $contents[] = array('text' => TEXT_INFO_EDIT_ACCOUNT_INTRO);
  $contents[] = array('align' => 'left', 'text' => '<div class="py-2">
  <div class="mb-1">'.TEXT_INFO_EDIT_ACCOUNT_INTRO.'</div>
  <a class="btn btn-primary" href="
    ' . tep_href_link(FILENAME_JOBSEEKER_REGISTER1, 'jID=' . $rInfo->jobseeker_id ) . '">
    Edit</a>&nbsp;
    <a class="btn btn-secondary" href="
    ' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('jID','selected_box'))).'&jID=' . $rInfo->jobseeker_id.'&action=delete' . '">
    Delete</a>');
  ...
  //                         <label class="font-weight-bolder">'.TEXT_INFO_UPDATED.'</label><h5>'.$rInfo->updated.'</h5></div>');
  // $contents[] = array('text' => '<div class="py-2">
  //                         <label class="font-weight-bolder">'.TEXT_INFO_LAST_LOGIN.'</label><h5>'.$rInfo->last_login_time.'</h5></div>');
  // $contents[] = array('text' => '<div class="py-2">
  //                         <label class="font-weight-bolder">'.TEXT_INFO_NUMBER_OF_LOGON.'</label><h5>'.$rInfo->number_of_logon.'</h5></div>');
 }
 break;
}
////
...
if($action1=='delete')
{
 $check_link='<br>'.TEXT_DELETE_WARNING.'<br><br><a href="#"  onclick="DeleteSelected(\'confirm_bulk_delete\')">'.tep_button('Confirm','class="btn btn-primary"').'</a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS).'">' . tep_button('Cancel','class="btn btn-primary"') . '</a>';
...
{
 $check_link='<a href="#" onclick="checkall()">Check All</a> / <a href="#"  onclick="uncheckall()">Uncheck All</a> <b>With Selected <a class="btn-text text-danger" href="#"  onclick="DeleteSelected(\'delete\')">Delete</a></b></font>';
 $template->assign_vars(array(
 'TABLE_HEADING_NAME'=>"<a href='".tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('sort','jID','selected_box','action')))."&sort=".$obj_sort_by_clause->return_sort_array['name'][0]."' class='white'>".TABLE_HEADING_NAME.$obj_sort_by_clause->return_sort_array['image'][0]."</a>",
 'TABLE_HEADING_EMAIL'=>"<a href='".tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('sort','jID','selected_box','action')))."&sort=".$obj_sort_by_clause->return_sort_array['name'][1]."' class='white'>".TABLE_HEADING_EMAIL.$obj_sort_by_clause->return_sort_array['image'][1]."</a>",
 'TABLE_HEADING_STATUS'=>"<a href='".tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('sort','jID','selected_box','action')))."&sort=".$obj_sort_by_clause->return_sort_array['name'][2]."' class='white'>".TABLE_HEADING_STATUS.$obj_sort_by_clause->return_sort_array['image'][2]."</a>",
 'TABLE_HEADING_FEATURED_STATUS'=>"<a href='".tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_PAID_JOBSEEKERS, tep_get_all_get_params(array('sort','jID','selected_box','action')))."&sort=".$obj_sort_by_clause->return_sort_array['name'][3]."' class='white'>".TABLE_HEADING_FEATURED_STATUS.$obj_sort_by_clause->return_sort_array['image'][3]."</a>",
  'TABLE_HEADING_TOTAL_RESUMES'=>TABLE_HEADING_TOTAL_RESUMES,
'TABLE_HEADING_ACTION'=>TABLE_HEADING_ACTION,
 'INFO_TEXT_STATUS'=>INFO_TEXT_STATUS,
 'INFO_TEXT_STATUS1'=>tep_draw_pull_down_menu('search_status', $search_status_array, $search_status1,'onchange="document.disply.submit();" class="form-control form-control-sm form-control form-control-sm-sm"'),
 'new_button'=>'<a class="btn btn-primary" href="' . tep_href_link(FILENAME_JOBSEEKER_REGISTER1, 'add=jobseeker') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> New</a>',
  ));
}
