 ...
 switch ($action) 
  ...
  case 'save':
   ...
   $country_name   = tep_db_prepare_input($_POST['TR_country_name']);
			$fr_country_name= tep_db_prepare_input($_POST['TR_fr_country_name']);
   ...
   $sql_data_array['country_name'] = $country_name;
			$sql_data_array['fr_country_name'] = $fr_country_name;
   ...
   $sql_data_array['priority'] = $priority;
			...
			if($action=='insert')
			{
				...
				else if($row_chek=getAnyTableWhereData(COUNTRIES_TABLE,"fr_country_name='".tep_db_input($fr_country_name)."'",'id'))
				{
    ...
    $id=(int)$_GET['id'];
				...
				else if($row_chek=getAnyTableWhereData(COUNTRIES_TABLE,"fr_country_name='".tep_db_input($fr_country_name)."' and id!='$id'",'id'))
				{
					$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
					$action='edit';
				}
...
///////////// Middle Values 
$country_query_raw="select c.id, c.continent_id, cont.continent_name, c.country_name, c.fr_country_name,c.country_code,c.priority from " . COUNTRIES_TABLE ." as c left join ".CONTINENT_TABLE." as cont on (c.continent_id=cont.id) order by c.country_name";
...
if(tep_db_num_rows($country_query) > 0)
{
 ...
 while ($country = tep_db_fetch_array($country_query)) 
 {
  ...
  $template->assign_block_vars('country', array( 'row_selected' => $row_selected,
   ...
   'fr_country_name' => tep_db_output($country['fr_country_name']),
...
switch ($action) 
{
 ...
 case 'save':
    ...
    '.tep_draw_input_field('TR_fr_country_name', $_POST['TR_fr_country_name'], 'class="form-control form-control-sm"' ).'
    </div>
    <div class="form-group">
 ...
 case 'edit':
  ...
  <div class="form-group">
  <label>'.TEXT_INFO_FR_COUNTRY_NAME.'</label>
  '.tep_draw_input_field('TR_fr_country_name', $cInfo->fr_country_name, 'class="form-control form-control-sm"' ).'
  </div>
  <div class="form-group">
  <label>'.TEXT_INFO_COUNTRY_CODE.'</label>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_COUNTRY, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
