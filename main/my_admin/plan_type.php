 ...
 switch ($action) 
  ...
  case 'save':
   ...
   $sql_data_array = array('plan_type_name'    => $plan_type_name,
                       				'fr_plan_type_name' => $fr_plan_type_name,
   ...
   $sql_data_array['sms']=(tep_not_null($_POST['ch_no_of_sms'])?'2147483647':tep_db_prepare_input($_POST['no_of_sms']));
			...
			if($action=='insert')
			{
				...
				else if($row_chek=getAnyTableWhereData(PLAN_TYPE_TABLE,"fr_plan_type_name='".tep_db_input($fr_plan_type_name)."'",'id'))
				{
    ...
    $id=(int)$_GET['id'];
				...
				else if($row_chek=getAnyTableWhereData(PLAN_TYPE_TABLE,"fr_plan_type_name='".tep_db_input($fr_plan_type_name)."' and id!='$id'",'id'))
				{
					$messageStack->add(MESSAGE_FR_NAME_ERROR, 'error');
					$action='edit';
				}
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_PLAN_TYPE, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
