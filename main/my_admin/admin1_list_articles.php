 ...
 switch($action)
 {
  ...
  case 'preview':
    ...
    if(!$error)
    {
     ...
     {
      if($obj_resume = new upload('article_photo', PATH_TO_MAIN_PHYSICAL_TEMP,'644',array('jpg','gif','png')))
      {
...
///only for sorting ends
 ...
 if(tep_db_num_rows($article_query) > 0)
 {
  ...
  while ($article = tep_db_fetch_array($article_query))
  {
   ...
   {
    $status='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, tep_get_all_get_params(array('id','action','selected_box'))).'&id=' . $article['id'] . '&action=article_inactive' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_ARTICLE_INACTIVATE, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_ARTICLE_ACTIVE, 30, 20);
   }
   else
   {
    $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_ARTICLE_INACTIVE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, tep_get_all_get_params(array('id','action','selected_box'))).'&id=' . $article['id'] . '&action=article_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_ARTICLE_ACTIVATE, 30, 20) . '</a>';
   }
   $template->assign_block_vars('article', array( 'row_selected' => $row_selected,
    'action' => $action_image,
    'status' => $status,
    'title' => tep_db_output($article['title']),
    'category' => $category_name,
    'show_date' => date("m-d-Y", strtotime($article['show_date'])),
    'inserted' => date("m-d-Y", strtotime($article['inserted'])),
    ));
  }
...
elseif($action=='new' || $action=='edit' || $action=='back')
{
 ...
 'INFO_TEXT_CATEGORY'=>INFO_TEXT_CATEGORY,
 'INFO_TEXT_CATEGORY1'=>get_drop_down_list1(ARTICLE_CATEGORY_TABLE,'name="TR_category" class="form-control form-control-sm"',"","",$category_id),
 'INFO_TEXT_AUTHOR'=>INFO_TEXT_AUTHOR,
...
else
{
 $template->assign_vars(array(
  ...
  'new_button'=>'<a class="btn btn-primary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_ARTICLES, 'action=new') . '"><i class="fa fa-plus" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
 ));
