...
switch ($action)
{
 ...
 default:
  if (is_object($nInfo))
  {
   ...
   $contents[] = array('text' => '<br>Date Added :' . date("m-d-Y", strtotime($nInfo->date_added)));
   if ($nInfo->status == '1')
    $contents[] = array('text' => 'Date Sent: ' . date("m-d-Y", strtotime($nInfo->date_sent)));
  }
 break;
}
...
else
{
 $template->assign_vars(array(
  ...
  'new_news_letter_button'=>'<a class="btn btn-primary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_NEWS_LETTER, 'action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> ' .IMAGE_NEW . '</a>',
