...
switch ($action)
{
 ...
 case 'save':
	...
	//	$contents[] = array('text' => '<br>' . TEXT_INFO_CONFIGURATION_NAME . '<br>'.tep_draw_input_field('TR_configuration_name',$configuration_name,));
		$contents[] = array('text' => '<div class="form-group"><label>' . TEXT_INFO_CONFIGURATION_VALUE . '</label>'.tep_draw_input_field('TR_configuration_value',$configuration_value,'class="form-control form-control-sm"').'</div>');
 ...
 case 'edit':
  ...
  else
  {
   if($cInfo->configuration_name=='MODULE_PAYMENT_MONEYORDER_PAYTO')
			{
    $value_field=tep_draw_textarea_field('TR_configuration_value', true,30,3,$cInfo->configuration_value);
			}
 ...
 default:
  if (isset($cInfo) && is_object($cInfo))
   ...
   $contents[] = array('text' => '<br>'.TEXT_INFO_DATE_ADDED. date("m-d-Y", strtotime($cInfo->inserted)));
   if (tep_not_null($cInfo->updated))
			 $contents[] = array('text' => '<br>' .TEXT_INFO_DATE_UPDATED. date("m-d-Y", strtotime($cInfo->updated)));
  }
  break;
}
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_CONFIGURATION, 'gid=' . $gid . '&action=new') .'"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '. IMAGE_NEW.'</a>',
