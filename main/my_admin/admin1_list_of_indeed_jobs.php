...
if($db_job_num_row > 0)
{
 ...
 while ($job = tep_db_fetch_array($db_job_query))
 {
  if($action1!='delete')
  {
   ...
   {
    $status='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_INDEED_JOBS, tep_get_all_get_params(array('jobID','action','selected_box'))).'&jobID='.$job['job_id'].'&action=job_inactive' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_JOB_INACTIVATE, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_JOB_ACTIVE, 30, 20);
   }
   else
   {
    $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOB_INACTIVE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_INDEED_JOBS, tep_get_all_get_params(array('jobID','action','selected_box'))).'&jobID='.$job['job_id'] . '&action=job_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOB_ACTIVATE, 30, 20) . '</a>';
   }
   if ($job['job_featured'] == 'Yes')
   {
    $featured='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_INDEED_JOBS, tep_get_all_get_params(array('jobID','action','selected_box'))).'&jobID=' . $job['job_id'] . '&action=job_not_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_JOB_NOT_FEATURED, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_JOB_FEATURED, 30, 20);
   }
   else
   {
    $featured=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOB_NOT_FEATURE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_INDEED_JOBS, tep_get_all_get_params(array('jobID','action','selected_box'))).'&jobID=' . $job['job_id'] . '&action=job_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOB_FEATURED, 30, 20) . '</a>';
   }
  }
  else
  {
 ...
 default:
  ...
  //  $contents[] = array('align' => 'center', 'text' => TEXT_INFO_EDIT_JOB_INTRO);
   $contents[] = array('align' => 'center', 'text' => '
              <div class="py-2">
                  <a class="btn btn-secondary" href="' . tep_href_link(FILENAME_RECRUITER_POST_JOB, 'rID=' . $jInfo->rec_id .'&jobID=' . $jInfo->job_id ) . '">
                          Edit
                  </a> 
                  <a class="btn btn-danger" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_INDEED_JOBS, 
