...
if(tep_db_num_rows($skill_tag_query) > 0)
{
 ...
 while ($skill_tag = tep_db_fetch_array($skill_tag_query))
 {
  ...
  {
   $status='<a href="'.tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SKILL_TAGS, tep_get_all_get_params(array('id','action','selected_box'))).'&id='.$skill_tag['id'].'&action=tag_inactive' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_TAG_INACTIVATE, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_TAG_ACTIVE, 30, 20);
  }
  else
  {
   $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif',STATUS_TAG_INACTIVE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SKILL_TAGS, tep_get_all_get_params(array('id','action','selected_box'))).'&id='.$skill_tag['id'].'&action=tag_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_TAG_ACTIVATE, 30, 20) . '</a>';
  }
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_STATUS1'=>tep_draw_pull_down_menu('skill_status', $skill_status_array, $skill_status1,'onchange="document.disply.submit();" class="form-control form-control-sm"'),
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_SKILL_TAGS, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
