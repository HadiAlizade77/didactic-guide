...
if (tep_not_null($action))
{
   ...
   if($check_status['invoices_status'] == "3")
   {
      ...
      'invoice_date'=>date("m-d-Y", strtotime($check_status['date_purchased'])),
...
if (($action == 'edit' || $action == 'print') && isset($_GET['iID']))
{
	...
	$invoice_exists = true;
 $row_comment=mysql_fetch_array($invoices_query);
 ...
 for ($i = 0, $n = sizeof($invoice->totals); $i < $n; $i++)
 {
  $invoice_total_text.='
	...
	$template->assign_vars(array(
		...
		'TABLE_HEADING_DATE_ADDED1'=>date("m-d-Y", strtotime($invoice->info['date_purchased'])),
...
else
{
	...
	$search_text=HEADING_TITLE_SEARCH . ' ' . tep_draw_input_field('iID', '', 'size="12"') . tep_draw_hidden_field('action', 'edit');
	$invoice_form=tep_draw_form('status', PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_INVOICES, '', 'get');
	$invoice_text=' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.HEADING_TITLE_STATUS . ' ' . tep_draw_pull_down_menu('status', array_merge(array(array('id' => '', 'text' => TEXT_ALL_INVOICES)), $invoices_statuses), $_GET['status'], 'class="form-control form-control-sm" onChange="this.form.submit();"');
	if (isset($_GET['cID']))
	{
...
//   'name' => '<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_INVOICES, tep_get_all_get_params(array('iID', 'action','selected_box')) . 'iID=' . $invoices['orders_id'] . '&action=edit') . '">' .  tep_db_output($invoices['recruiter_name']) . '</a>' ,
   ...
   'date_purchased' => date("m-d-Y", strtotime($invoices['date_purchased'])),
...
switch ($action)
{
	...
	default:
		...
		{
			$heading[] = array('text' => '<b>[' . $iInfo->orders_id . ']&nbsp;&nbsp;' . date("m-d-Y", strtotime($iInfo->date_purchased)) . '</b>');
			...
			$contents[] = array('text' => '<br>' . TEXT_DATE_INVOICE_CREATED . ' ' . date("m-d-Y", strtotime($iInfo->date_purchased)));
			if (tep_not_null($iInfo->last_modified))
				$contents[] = array('text' => TEXT_DATE_INVOICE_LAST_MODIFIED . ' ' . date("m-d-Y", strtotime($iInfo->last_modified)));
