...
if(tep_not_null($search_status1))
{
 ...
 elseif($search_status1=='inactive')
 {
  $db_recruiter_query_raw = "select rl.recruiter_id, concat(r.recruiter_first_name,' ',r.recruiter_last_name) as recruiter_name, r.recruiter_featured, recruiter_applywithoutlogin, rl.recruiter_email_address, r.recruiter_company_name, rl.recruiter_status from " . RECRUITER_LOGIN_TABLE . " as rl, " . RECRUITER_TABLE . " as r where rl.recruiter_id=r.recruiter_id and rl.recruiter_email_address!='demo1@aynsoft.com' and recruiter_status='No' $whereClause order by ".$order_by_clause;
            ...
            ) as c where c.recruiter_name like '%$search_query%' or c.recruiter_email_address like '%$search_query%' order by c.inserted";
...
if($db_recruiter_num_row > 0)
{
 ...
 while ($recruiter = tep_db_fetch_array($db_recruiter_query))
 {
  if($action1!='delete')
  {
   ...
   {
    $status='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITERS, tep_get_all_get_params(array('rID','action','selected_box'))).'&rID=' . $recruiter['recruiter_id'] . '&action=recruiter_inactive' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_RECRUITER_INACTIVATE, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_RECRUITER_ACTIVE, 30, 20);
   }
   else
   {
    $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_RECRUITER_INACTIVE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITERS, tep_get_all_get_params(array('rID','action','selected_box'))).'&rID=' . $recruiter['recruiter_id'] . '&action=recruiter_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_RECRUITER_ACTIVATE, 30, 20) . '</a>';
   }
   if ($recruiter['recruiter_featured'] == 'Yes')
   {
    $featured_status='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITERS, tep_get_all_get_params(array('rID','action','selected_box'))).'&rID=' . $recruiter['recruiter_id'] . '&action=recruiter_not_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_RECRUITER_NOT_FEATURED, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_RECRUITER_FEATURED, 30, 20);
   }
   else
   {
    $featured_status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_RECRUITER_NOT_FEATURE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITERS, tep_get_all_get_params(array('rID','action','selected_box'))).'&rID=' . $recruiter['recruiter_id'] . '&action=recruiter_featured' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_RECRUITER_FEATURED, 30, 20) . '</a>';
   }
   if ($recruiter['recruiter_applywithoutlogin'] == 'Yes')
   {
    $applywithoutlogin='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITERS, tep_get_all_get_params(array('rID','action','selected_box'))).'&rID=' . $recruiter['recruiter_id'] . '&action=recruiter_notapplywithoutlogin' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_RECRUITER_NOTAPPLYWITHOUTLOGIN, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_RECRUITER_APPLYWITHOUTLOGIN, 30, 20);
   }
   else
   {
    $applywithoutlogin=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_RECRUITER_INACTIVE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITERS, tep_get_all_get_params(array('rID','action','selected_box'))).'&rID=' . $recruiter['recruiter_id'] . '&action=recruiter_applywithoutlogin' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_RECRUITER_ACTIVATE, 30, 20) . '</a>';
   }
...
switch ($action)
{
 ...
 {
  $heading[] = array('text' => '<div class=""><div>'.$rInfo->recruiter_name.'
                      <div class="font-weight-bold mb-2">
                      '.TEXT_COMPANY . ' ' .$rInfo->recruiter_company_name.'
                      </div>
                      <div>'.TEXT_INFO_EDIT_ACCOUNT_INTRO.'</div>
                      </div>
                      </div>
                    ');
  ...
  $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"',
                      'text' => '<a class="list-group-item list-group-item-action mt-3" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_JOBS, 'rID=' . $rInfo->recruiter_id ).'" class="right_black"><i class="fa fa-caret-right" aria-hidden="true"></i> '.IMAGE_JOBS.'</a>');
  $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow1" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"',
                      'text' => '<a class="list-group-item list-group-item-action" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ORDERS, 'rID=' . $rInfo->recruiter_id ).'" class="right_black"><i class="fa fa-caret-right" aria-hidden="true"></i> '.IMAGE_ORDERS.'</a>');
  $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"',
                      'text' => '<a class="list-group-item list-group-item-action" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ACCOUNTS, 'plan_for=job_post&rID=' . $rInfo->recruiter_id ).'" class="right_black"><i class="fa fa-caret-right" aria-hidden="true"></i> '.IMAGE_RECRUITER_ACCOUNT_JOB.'</a>');
  $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow1" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"',
                      'text' => '<a class="list-group-item list-group-item-action" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ACCOUNTS, 'plan_for=resume_search&rID=' . $rInfo->recruiter_id ).'" class="right_black"><i class="fa fa-caret-right" aria-hidden="true"></i> '.IMAGE_RECRUITER_ACCOUNT_RESUME.'</a>');
  $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"',
                      'text' => '<a class="list-group-item list-group-item-action" href="' . tep_href_link(FILENAME_RECRUITER_REGISTRATION, 'rID=' . $rInfo->recruiter_id ).'" class="right_black"><i class="fa fa-caret-right" aria-hidden="true"></i> '.IMAGE_EDIT.'</a>');
  $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow1" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"',
                      'text' => '<a class="list-group-item list-group-item-action" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITERS, tep_get_all_get_params(array('rID','selected_box'))).'&rID=' . $rInfo->recruiter_id.'&action=delete'.'" class="right_black"><i class="fa fa-caret-right" aria-hidden="true"></i> '.IMAGE_DELETE.'</a>');
  $contents[] = array('align' => 'left', 'params'=>'class="dataTableRightRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)"',
                      'text' => '<a class="list-group-item list-group-item-action" href="' . tep_href_link(FILENAME_RECRUITER_CONTROL_PANEL, 'rID=' . $rInfo->recruiter_id ).'" class="right_black"><i class="fa fa-caret-right" aria-hidden="true"></i> '.INFO_TEXT_RECRUITER_CONTROL_PANEL.'</a>');
  $contents[] = array('params'=>'class="dataTableRightRow1"', 'text' => '<div class="py-2">
...
// $check_link='<br>'.TEXT_DELETE_WARNING.'<br><br><a href="#"  style="color:#0000ff" onclick="DeleteSelected(\'confirm_bulk_delete\')"><button class="btn btn-primary">Confirm</button></a>&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITERS).'"><button class="btn btn-secondary">Cancel</button></a>';
 $check_link='<br>'.TEXT_DELETE_WARNING.'<br><br><a class="btn-text text-danger" href="#" onclick="DeleteSelected(\'confirm_bulk_delete\')" type="button" class="btn btn-primary">Confirm</a>&nbsp;<button onclick="location.href=\''.tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITERS).'\'" type="submit" class="btn btn-secondary">Cancel</a>';
...
'TABLE_HEADING_NO_OF_ACTIVE_JOBS_POSTED'=>TABLE_HEADING_NO_OF_ACTIVE_JOBS_POSTED,
 ...
 <i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> New
                </a>',
