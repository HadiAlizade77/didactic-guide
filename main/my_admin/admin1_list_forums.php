 ...
 if(tep_db_num_rows($forum_query) > 0)
 {
  ...
  while ($forum = tep_db_fetch_array($forum_query))
  {
   ...
   $template->assign_block_vars('forum', array( 'row_selected' => $row_selected,
    ...
    'show_date' => date("m-d-Y", strtotime($forum['show_date'])),
    'inserted' => date("m-d-Y", strtotime($forum['inserted'])),
    ));
  }
...
else
{
 $template->assign_vars(array(
  ...
  'new_button'                    => '<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_FORUMS, 'action=new') . '"><i class="fa fa-plus" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
 ));
