...
if($action!="")
{
 switch ($action)
  ...
  case 'save1':
   ...
   $fr_category_name=tep_db_prepare_input($_POST['TR_fr_article_category_name']);
   $fr_sub_category_name=tep_db_prepare_input($_POST['TR_fr_article_sub_category_name']);
   $sql_data_array['category_name'] = $category_name;
			...
			if($action=='insert')
			{
				...
				elseif($row_chek=getAnyTableWhereData(ARTICLE_CATEGORY_TABLE,"fr_category_name='".tep_db_input($fr_category_name)."'",'id'))
				{
					$messageStack->add(MESSAGE_NAME_ERROR, 'error');
				}
				else
     ...
     tep_db_perform(ARTICLE_CATEGORY_TABLE, $sql_data_array);
			...
			else if($action=='insert1')
			{
				...
				else if($row_chek=getAnyTableWhereData(ARTICLE_CATEGORY_TABLE,"fr_category_name='".tep_db_input($fr_sub_category_name)."'",'id'))
				{
					$messageStack->add(MESSAGE_NAME_ERROR, 'error');
				}
				else
    ...
    $id=(int)$_GET['id'];
				...
				else if($row_chek=getAnyTableWhereData(ARTICLE_CATEGORY_TABLE,"fr_category_name='".tep_db_input($fr_category_name)."' and id!='$id'",'id'))
				{
					$messageStack->add(MESSAGE_NAME_ERROR, 'error');
					$action='edit';
				}
    ...
    $id=(int)$_GET['id'];
				...
				else if($row_chek=getAnyTableWhereData(ARTICLE_CATEGORY_TABLE,"fr_category_name='".tep_db_input($fr_sub_category_name)."' and id!='$id'",'id'))
				{
					$messageStack->add(MESSAGE_NAME_ERROR, 'error');
					$action='edit';
				}
...
///////////// Middle Values
$article_category_query_raw="select id, sub_cat_id, category_name,fr_category_name from " . ARTICLE_CATEGORY_TABLE ." order by category_name asc";
...
if(tep_db_num_rows($article_category_query) > 0)
{
 ...
 while ($article_category = tep_db_fetch_array($article_category_query))
 {
		...
		if($article_category['sub_cat_id']!='')
  {
   ...
   while($sub_cat_id!="")
   {
    $row_sub_cat=getAnyTableWhereData(ARTICLE_CATEGORY_TABLE,"id='".$sub_cat_id."'","fr_category_name,sub_cat_id");
    $sub_cat_id=$row_sub_cat['sub_cat_id'];
   ...
   $fr_category_name=$fr_category_name." -> ".$article_category['fr_category_name'];
  }
  else
  {
   $fr_category_name=$article_category['fr_category_name'];
  }
...
switch ($action)
{
 ...
 case 'insert1':
  ...
  '.tep_draw_input_field('TR_fr_article_category_name', ($action=='insert'?$_POST['TR_fr_article_category_name']:''), 'class="form-control form-control-sm"' ).'
  </div>
  ...
  '.tep_draw_input_field('TR_fr_article_sub_category_name', $_POST['TR_fr_article_sub_category_name'], 'class="form-control form-control-sm"' ).'
  </div>
  '.tep_draw_submit_button_field('',IMAGE_INSERT,'class="btn btn-primary"').'
  <a class="btn btn-secondary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_ARTICLE_CATEGORIES).'">'.IMAGE_CANCEL.'</a>
  </form>
 ...
 case 'save1':
  ...
  //  $contents[] = array('align' => 'left', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif',IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_ARTICLE_CATEGORIES, 'gid=' . $_GET['gid'] . '&id=' . $cInfo->id ). '">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif',IMAGE_CANCEL).'</a>');
   ...
   <div class="form-group">
   <label>'.TEXT_INFO_FR_ARTICLE_SUB_CATEGORY_NAME.'</label>
   '.tep_draw_input_field('TR_fr_article_sub_category_name', ($action=='save1'?$_POST['TR_fr_article_sub_category_name']:$cInfo->fr_category_name), 'class="form-control form-control-sm"' ).'
   </div>
  ...
  //  $contents[] = array('align' => 'left', 'text' => '<br>'.tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE).'&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_ARTICLE_CATEGORIES).'">'.tep_image_button(PATH_TO_BUTTON.'button_cancel.gif', IMAGE_CANCEL).'</a>');
   ...
   <div class="form-group">
   <label>'.TEXT_INFO_FR_ARTICLE_CATEGORY_NAME.'</label>
   '.tep_draw_input_field('TR_fr_article_category_name', ($action=='save'?$_POST['TR_fr_article_category_name']:$cInfo->fr_category_name), 'class="form-control form-control-sm"' ).'
   </div>
   '.tep_draw_submit_button_field('',IMAGE_UPDATE,'class="btn btn-primary"').'
   <a class="btn btn-secondary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_ARTICLE_CATEGORIES).'">'.IMAGE_CANCEL.'</a>
   </form>
...
$template->assign_vars(array(
 ...
 'new_button'=>'<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_ADMIN_ARTICLE_CATEGORIES, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
