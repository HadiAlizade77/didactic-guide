  ...
  case 'ping_sitemap':
     ...
     if(!$ping_error)
     {
      ...
      for($i=0;$i<$total_site;$i++)
      {
       ...
       {
        case 'google':
         $url= 'http://www.google.com/webmasters/tools/ping?sitemap='.urlencode($sitemap_url);
        ...
        case 'msn':
         ...
         break;
        case 'bing':
         $url= 'http://www.bing.com/webmaster/ping.aspx?siteMap='.urlencode($sitemap_url);
         $header =tep_site_map_submission($url);
         if($header['http_code']=='200' && preg_match("/Thanks for submitting your sitemap/si",$header['output']))
         {
...
$template->assign_vars(array(
 ...
 'new_button'               => '<a class="btn btn-primary float-right" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_SITE_MAP, 'page=' . $_GET['page'] .'&action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> '.IMAGE_NEW.'</a>',
