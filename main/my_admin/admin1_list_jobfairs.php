...
///only for sorting ends
 ...
 if(tep_db_num_rows($jobfair_query) > 0)
 {
  ...
  while ($jobfair = tep_db_fetch_array($jobfair_query))
  {
   ...
   {
    $status='<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_JOBFAIRS, tep_get_all_get_params(array('id','action','selected_box'))).'&id=' . $jobfair['id'] . '&action=jobfair_inactive' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_red_light.gif', STATUS_JOBFAIR_INACTIVATE, 30, 20) . '</a>&nbsp;' . tep_image(PATH_TO_IMAGE.'icon_status_green.gif', STATUS_JOBFAIR_ACTIVE, 30, 20);
   }
   else
   {
    $status=tep_image(PATH_TO_IMAGE.'icon_status_red.gif', STATUS_JOBFAIR_INACTIVE, 30, 20) . '&nbsp;<a href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_JOBFAIRS, tep_get_all_get_params(array('id','action','selected_box'))).'&id=' . $jobfair['id'] . '&action=jobfair_active' . '">' . tep_image(PATH_TO_IMAGE.'icon_status_green_light.gif', STATUS_JOBFAIR_ACTIVATE, 30, 20) . '</a>';
   }
...
else
{
 $template->assign_vars(array(
  ...
  'new_button'=>'<a class="btn btn-primary" href="' . tep_href_link(PATH_TO_ADMIN.FILENAME_ADMIN1_LIST_OF_JOBFAIRS, 'action=new') . '"><i class="fa fa-plus fa-admin-icons" aria-hidden="true"></i> ' .IMAGE_NEW . '</a>',
 ));
