  ...
  case 'back':
   ...
   if(strlen($fr_description)<=0)
    ...
    $messageStack->add(MESSAGE_FR_DESCRIPTION_ERROR, 'error');
     ...
     fwrite($handle, stripslashes($row_template['description']));
     fclose($handle);
     $handle1 = fopen(PATH_TO_MAIN_PHYSICAL.PATH_TO_TEMPLATE.'fr_application_send_template.htm', "w");
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_FR_APPLICATION_SEND_TEMPLATE_DESCRIPTION1'=>tep_draw_textarea_field('fr_description2', 'soft', '', '10', $fr_description, ' id="fr_description2"', true, true),
 'HOST_NAME'                       =>HOST_NAME, 
 ...
 else if($action=='preview' || $action=='save' || $action=='insert')
 {
  $template->assign_vars(array(
   ...
   'INFO_TEXT_FR_APPLICATION_SEND_TEMPLATE_DESCRIPTION'=>INFO_TEXT_FR_APPLICATION_SEND_TEMPLATE_DESCRIPTION,
   'INFO_TEXT_FR_APPLICATION_SEND_TEMPLATE_DESCRIPTION1'=>stripslashes($fr_description),
