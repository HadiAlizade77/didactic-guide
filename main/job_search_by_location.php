...
$template->set_filenames(array('job_search' => 'job_search_by_location.htm','job_search_states'=>'job_search_by_location1.htm','job_search_result'=>'job_search_result.htm'));
...
$search_name='';
// initialize
list($country_id, $country_name, $fr_country_name) = (isset($_GET['country'])?array_values(decode_country($_GET['country'])):'');
list($zone_id, $zone_country_id,$zone_code , $zone_name, $fr_zone_name) = (isset($_GET['state'])?array_values(decode_state($_GET['state'])):'');
...
if($action!='search' && $_GET['country']=='')
{
		...
		$continent_query = "select $field_names from ".CONTINENT_TABLE." $whereClause order by continent_name asc ";
   ...
   while($country = tep_db_fetch_array($country_result))
    ...
    $no_of_jobs=$jobs_row['x1'];
				...
				if($j%3==0)
				{
					$country_list.='<td><a href="'.tep_href_link(encode_forum($continent_name).'/'.encode_forum($country[TEXT_LANGUAGE.'country_name']).'/').'" title="'.$country[TEXT_LANGUAGE.'country_name'].'" class="style27">'.$country[TEXT_LANGUAGE.'country_name'].'</a><span class="style27">'.$jobs.'</span></td>';
				}
				else if($j%3==1)
				{
					$country_list.='<td><a href="'.tep_href_link(encode_forum($continent_name).'/'.encode_forum($country[TEXT_LANGUAGE.'country_name']).'/').'" title="'.$country[TEXT_LANGUAGE.'country_name'].'" class="style27">'.$country[TEXT_LANGUAGE.'country_name'].'</a><span class="style27">'.$jobs.'</span></td>';
				...
				{
					$country_list.='<td><a href="'.tep_href_link(encode_forum($continent_name).'/'.encode_forum($country[TEXT_LANGUAGE.'country_name']).'/').'" title="'.$country[TEXT_LANGUAGE.'country_name'].'" class="style27">'.$country[TEXT_LANGUAGE.'country_name'].'</a><span class="style27">'.$jobs.'</span></td></tr>';
				}
				$j++;
			}
			$country_list.='</table>';
 ...
 $template->assign_vars(array(
  ...
  'button' => '<button class="btn btn-sm btn-primary" type="submit" id="button-addon2">Search</button>',//tep_image_submit(PATH_TO_BUTTON.'button_search.gif', IMAGE_SEARCH),
  ...
  'INFO_TEXT_COUNTRY1'  => LIST_SET_DATA(COUNTRIES_TABLE,"",TEXT_LANGUAGE.'country_name','id',"country_name",'name="country" class="form-control" ',INFO_TEXT_ALL_COUNTRIES,'',$country),
...
elseif($action!='search' && $_GET['country']!='')
{
	 $state_query  = "select * from ".ZONES_TABLE." where zone_country_id='".$country_id."' order by zone_name  asc";
        ...
        '<button type="submit" class="btn btn-link text-gray">'.$states[TEXT_LANGUAGE.'zone_name'].$jobs.'</button></form></td>';
			}
			else if($s%3==1)
			{
				// $state_list.='<td><a href="'.tep_href_link(encode_forum($continent_name).'/'.encode_forum($country_name).'/'.encode_forum($states[TEXT_LANGUAGE.'zone_name']).'/').'" title="'.$states[TEXT_LANGUAGE.'zone_name'].'" class="style27">'.$states[TEXT_LANGUAGE.'zone_name'].'</a><span class="style27">'.$jobs.'</span></td>';
        ...
        .tep_draw_hidden_field('action','search')
        .tep_draw_hidden_field('keyword',$states[TEXT_LANGUAGE.'zone_name']).
        '<button type="submit" class="btn btn-link text-gray">'.$states[TEXT_LANGUAGE.'zone_name'].$jobs.'</button></form></td></tr>';
			}
			$s++;
  }
 		$state_list.='</table>';
 ...
 $template->assign_vars(array(
		'HEADING_TITLE'       => HEADING_TITLE,
  'button' => '<button class="btn btn-primary" type="submit">Search</button>',//tep_image_submit(PATH_TO_BUTTON.'button_search.gif', IMAGE_SEARCH),
  // 'form' => tep_draw_form('search', FILENAME_JOB_SEARCH_BY_LOCATION,($edit?'sID='.$save_search_id:''),'post').tep_draw_hidden_field('action','search'),
  'form' => tep_draw_form('search', FILENAME_JOB_SEARCH,($edit?'sID='.$save_search_id:''),'post').tep_draw_hidden_field('action','search'),
  'INFO_TEXT_COUNTRY'   => INFO_TEXT_COUNTRY,
