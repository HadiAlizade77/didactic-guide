...
#################FEATURED EMPLOYER############################
$feat_emp=banner_display("7",6,130,'class="img-fluid featured-employer-theme6"');
...
$field_names="j.job_id, j.job_title, j.job_type, j.job_salary, j.job_location,j.job_featured,j.job_short_description,j.inserted,j.min_experience,j.max_experience,j.re_adv,r.recruiter_company_name,job_country_id,r.recruiter_logo";
...
/////logo
 ...
 if(tep_not_null($company_logo) && is_file(PATH_TO_MAIN_PHYSICAL.PATH_TO_LOGO.$company_logo))
     $recruiter_logo=tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_LOGO.$company_logo."&size=150",'','','','class="jobs-logo-theme6"');
else
     $recruiter_logo=tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_IMG."nologo.jpg"."&size=80",'','','','class="jobs-logo-theme6"');
...
{
	$save_search= tep_draw_form('save_search', FILENAME_JOB_ALERT_AGENT,($edit?'sID='.$save_search_id:''),'post','onsubmit="return ValidateForm(this)" class="d-flex mx-auto"').tep_draw_hidden_field('action1','save_search');
    $INFO_TEXT_ALERT_TEXT=$save_search.(($action1=='save_search')?'':"<a href='#' onclick='document.save_search.submit();' class='btn btn-lg btn-outline-secondary alert-search-btn-control bg-light text-dark'>Create job alert</a></form>");
}
else
{
	 $save_search= tep_draw_form('save_search', FILENAME_JOB_ALERT_AGENT_DIRECT,'','post','onsubmit="return ValidateForm(this)" class="d-flex mx-auto"').tep_draw_hidden_field('action','new');
	 $INFO_TEXT_ALERT_TEXT=$save_search.''.tep_draw_input_field('TREF_job_alert_email', $TREF_job_alert_email,'class="form-control form-control-lg alert-search-control rounded-0" placeholder="Enter email address" ',false).''."<button type='submit' class='btn btn-lg btn-success rounded-0 alert-search-btn-control text-dark subscribe bg-white'>Subscribe</button></form>";
}
...
'JOBSEEKER_SIGN_UP'=>(check_login('jobseeker')?'<button class="btn btn-outline-secondary-home" onclick="location.href=\''.tep_href_link(FILENAME_LOGOUT).'\'" type="submit">'.SIGN_OUT.'</button>':'<button class="btn btn-outline-secondary-home" onclick="location.href=\''.tep_href_link(FILENAME_JOBSEEKER_REGISTER1).'\'" type="submit">'.SIGN_UP.'</button>'),
'RECRUITER_SIGN_UP'=>(check_login('recruiter')?'<button class="btn btn-outline-secondary-home" onclick="location.href=\''.tep_href_link(FILENAME_LOGOUT).'\'" type="submit">'.SIGN_OUT.'</button>':'<button class="btn btn-outline-secondary-home" onclick="location.href=\''.tep_href_link(FILENAME_RECRUITER_REGISTRATION).'\'" type="submit">'.SIGN_UP.'</button>'),
'ADVERTISER_SIGN_UP'=>'<button class="btn btn-outline-secondary-home" onclick="location.href=\''.tep_href_link(FILENAME_CONTACT_US).'\'" type="submit">Contact Us</button>',
...
'SUBMIT_RESUME_TEXT'=>SUBMIT_RESUME_TEXT,
 'LEFT_BOX_WIDTH'=> '',
 'ALL_JOBS'=>tep_draw_form('search_job', FILENAME_JOB_SEARCH,'','post').tep_draw_hidden_field('action','search').'<button class="btn btn-outline-secondary-home" type="submit">'.ALL_JOBS.'</button></form>',
 'ALL_CATEGORY'=>'<button class="btn btn-outline-secondary-home" onclick="location.href=\''.tep_href_link(FILENAME_JOB_SEARCH_BY_INDUSTRY).'\'" type="submit">'.ALL_CATEGORIES.'</button>',
 'ALL_RECRUITERS'=>'<button class="btn btn-outline-secondary-home" onclick="location.href=\''.tep_href_link(FILENAME_JOBSEEKER_COMPANY_PROFILE).'\'" type="submit">'.ALL_RECRUITERS.'</button>',
