...
include_once(PATH_TO_MAIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_BULK_APPLY_NOW);
$template->set_filenames(array('apply_now' => 'apply_now.htm','email'=>'application_send_template.htm','fr_email'=>'fr_application_send_template.htm'));
...
//echo $_SESSION['sess_jobseekerid'];
if($row=getAnyTableWhereData(JOBSEEKER_TABLE,"jobseeker_id='".$_SESSION['sess_jobseekerid']."' and jobseeker_first_name=''",'jobseeker_id')) 
{
	$_SESSION['REDIRECT_URL']=$_SERVER['REQUEST_URI'];
 $messageStack->add_session(ERROR, 'error');
 tep_redirect(FILENAME_JOBSEEKER_REGISTER2);
...
$job_display_id_array=array();
$count_job=count($query_string1);
$display_job_title='';
for($i=0;$i<$count_job;$i++)
{
 $job_id=check_data($query_string1[$i],"=","job_id","job_id");
 ...
 $field_names="j.job_title,j.job_id,j.display_id,j.post_url,j.url";
 if($row=getAnyTableWhereData($table_names,$where_clause,$field_names)) 
 {
  ...
  $new_query_string[]=$query_string1[$i];
  $job_id_array[]=$job_id;
  $job_title_array[]=$row['job_title'];
  $display_job_title.="<li class='fw-bold text-success'>".tep_db_output($row['job_title'])."</li>";
  $job_display_id_array[]=$row['display_id'];
 }
}
$total_job=count($new_query_string);
if(count($total_job)<=0)
{
 $messageStack->add_session(ERROR_JOB_NOT_EXIST, 'error');
 tep_redirect(tep_href_link(FILENAME_ERROR));
}

$new_query_string1=implode(',',$new_query_string);
...
if(tep_not_null($action))
{
   ...
   else
   {
    ...
    tep_redirect(tep_href_link(FILENAME_BULK_APPLY_NOW,'query_string='.$new_query_string1));
    //$resume_id='0';
   }
   ...
   //////// file upload ends //////
   $success_apply=0;
   for($jobcount=0;$jobcount<$total_job;$jobcount++)
   {
    $job_id=check_data($new_query_string[$jobcount],"=","job_id","job_id");
    $now=date('Y-m-d H:i:s');
    $table_names=JOB_TABLE." as j, ".RECRUITER_LOGIN_TABLE." as rl,".RECRUITER_TABLE." as r";
    $where_clause=" j.recruiter_id=r.recruiter_id and rl.recruiter_id=r.recruiter_id and j.job_id='".$job_id."' and j.expired >='$now' and j.re_adv <='$now' and j.job_status='Yes' and ( j.deleted is NULL or j.deleted='0000-00-00 00:00:00')";
    $field_names="j.recruiter_user_id,j.job_reference,j.job_title,rl.recruiter_email_address,concat(r.recruiter_first_name,' ',r.recruiter_last_name) as r_full_name,display_id";
    $row=getAnyTableWhereData($table_names,$where_clause,$field_names); 
    ...
    if($row_screener=getAnyTableWhereData(SCREENER_TABLE,"job_id='".$job_id."'"))
    {
     ...
     for($i=1;$i<=NO_OF_SCREENERS;$i++)
     {
      ...
      $ans=INFO_TEXT_ANSWER .$display_id.'_'.$i;
      if($_POST[$ans]!='' && $ques!='')
      {
     ...
     if($k >0)
      $before_screen_string='<hr>'.INFO_TEXT_THE_CANDIDATE_RESPOND.' <font color="red"><b>'.$k.' / '.$no_of_screener.'</b></font>'.INFO_TEXT_QUALIFICATION_QUESTION."<br><br>";
    }
    //mail to recruiter
    //$message = new email();
    ...
    $email_subject=INFO_TEXT_JOBSITE_APPLICATION .tep_db_output($row['job_reference'])." - ".tep_db_output($row['job_title']);
    // Send message
...
{
 $screeners_array=array();
 $z=0;
 for($j=0;$j<$total_job;$j++)
 {
  $job_id = $job_id_array[$j];
  $display_id =$job_display_id_array[$j];

  if($row_screener=getAnyTableWhereData(SCREENER_TABLE,"job_id='".$job_id."'"))
  {
   $screeners_array[$z]='
                        <div class="col-md-12 text-center font-weight-bold"> '.($job_title_array[$j]). INFO_TEXT_SCREENER.'</div>';

   for($i=0;$i<NO_OF_SCREENERS;$i++)
   {
    if(tep_not_null($row_screener[q.($i+1)]))
    $screeners_array[$z].='
    <label for="staticEmail" class="col-sm-3 mb-2 col-form-label text-right">'.INFO_TEXT_QUESTION .($i+1).' :</label>
			<div class="col-sm-9 font-weight-bold">
			'.tep_db_output($row_screener[q.($i+1)]).'
			</div>

			<label for="staticEmail" class="col-sm-3 col-form-label text-right">'.INFO_TEXT_ANSWER .($i+1).' :</label>
			<div class="col-sm-9 font-weight-bold">
				<input type="text" class="form-control mb-2" name="answer'.$display_id.'_'.($i+1).'" size="60">
      </div>';
   }
   $z++;
  }
...
$screeners='';
if(count($screeners_array)>0)
 $screeners=implode('',$screeners_array);
}
$cover_letters='';
...
if($db_cover_letter_num_row > 0)
{
 ...
 while ($cover_letter = tep_db_fetch_array($db_cover_letter_query)) 
 {
  ...
  $cover_letters_array[]=tep_draw_radio_field('cover_letter', $ide, '', $ide, 'id="radio_cover_letter'.$ide.'"').'&nbsp;<label for="radio_cover_letter'.$ide.'" onMouseOver="this.style.color=\'#0000ff\'" onMouseOut="this.style.color=\'#000080\'">'.'<a href="#" onclick="popUp(\''.FILENAME_JOBSEEKER_PREVIEW_COVER_LETTER.'?cID='.$ide.'\')">'.tep_db_output($cover_letter['cover_letter_name']).'</a></label>&nbsp;';
 }
 if(tep_not_null($cover_letters_array))
 {
  $cover_letters=implode("<br>",$cover_letters_array);
 }
}
else
{
 $cover_letters=tep_draw_textarea_field('TR_cover_letter', 'soft', '50', '5', INFO_TEXT_DEFALUT_COVER_LETTER, '', true, true, 'class="form-control"');
}
...
if($db_resume_name_num_row > 0)
{
 while ($resume_name = tep_db_fetch_array($db_resume_name_query)) 
 {
  ...
  $resume_names_array[]=tep_draw_radio_field('my_resume_id', $ide, '', $ide, 'id="radio_resume_name'.$ide.'" class="form-check-input"').'&nbsp;<label class="form-check-label" for="radio_resume_name'.$ide.'" onMouseOver="this.style.color=\'#0000ff\'" onMouseOut="this.style.color=\'#000080\'">'.'<a href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME."?resume=".$query_string1).'" target="_resume">'.tep_db_output($resume_name['resume_title']).'</a></label>&nbsp;';
 }
}
else
{
 $resume_names=INFO_TEXT_NOT_ADDED_YET."&nbsp;&nbsp;<a href='".tep_href_link(FILENAME_JOBSEEKER_RESUME1)."' class='btn btn-outline-primary'>".INFO_TEXT_ADD_NEW_RESUME."</a>";
}
if(tep_not_null($resume_names_array))
{
 $resume_names=implode("<br>",$resume_names_array);
}
...
$template->assign_vars(array(
 ...
 'form'=>tep_draw_form('application', FILENAME_BULK_APPLY_NOW, 'query_string='.$new_query_string1, 'post', 'enctype="multipart/form-data" onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('action','apply'),
 ...
 'INFO_TEXT_DISPLAY_JOBS_TITLE'=>"<tr><td  valign='top' colspan='3'><hr></td></tr><tr><td  valign='top'><b><u>".INFO_TEXT_JOB_TITLE." :</u></b></td><td align='left' colspan='2'>".stripslashes($display_job_title)."</td></tr>",
//  'button'=>tep_image_submit(PATH_TO_BUTTON.'button_confirm.gif', IMAGE_CONFIRM),
 'button'=>tep_button_submit('btn btn-lg btn-primary px-4', 'Confirm'),
 'screeners'=>$screeners,
 'INFO_TEXT_DEFALUT_COVER_LETTER1'=>sprintf("<a href='".tep_href_link(FILENAME_JOBSEEKER_LIST_OF_COVER_LETTERS, 'action=add_new')."' class='text-primary'><svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-plus' viewBox='0 0 16 16'>
 <path d='M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z'/>
</svg> ".INFO_TEXT_ADD_NEW_COVER_LETTER."</a>"),
