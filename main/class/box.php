  ...
  if(($heading[0]['default_row'])==1)
  {
  	$this->table_row_parameters = 'class="menuBoxHeading1ddd"';
		...
		if (isset($heading[0]['link']))
		{
			...
			$heading[0]['text'] = '<div style="padding:8px 0;">'.$text_image.'<a href="' . $heading[0]['link'] . '" '.((($heading[0]['default_row'])==1)?' class="menuBoxHeadingLink1"':'class="menuBoxHeadingLink" ').' >' . $heading[0]['text'] . '</a></div>';		}
		else
		{
		...
		$this->table_data_parameters = 'class="menuBoxContent py-1"';
