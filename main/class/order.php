...
class order
{
  ...
  else if($products['discount_percentage']>0)
  {
   if(CONF_ORDER_TAX>0)
   {
    if(CONF_ORDER_TAX_CASE==1)
	{
	 ...
	 $this->info['subtotal_symbol'].="<br>Tax :".$tax."<br>";
 ...
 function get_products()
 {
  ...
  {
   if(!$row=getAnyTableWhereData(GIFT_USED_TABLE,"recruiter_id='".$_SESSION['sess_recruiterid']."' and gift_id='".tep_db_input($obj_gift->gift_id)."'","*",false))
   {
