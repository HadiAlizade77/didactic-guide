...
function get_list_of_tests($of_emp = null)
{
    ...
    if (tep_db_num_rows($test_result) > 0) {
        while ($data = tep_db_fetch_array($test_result)) {
            ...
            $title = '<div class="card-header card-header-custom border-bottom">'.$short_title.'</div>';
...
function actionBtn(int $id)
{
    $button = '
    <div class="btn-group">
        <button type="button" class="btn btn-outline-secondary btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
            Action
        </button>
...
function recuriter_assessment_list($recruiter_id)
{
    ...
    if (tep_db_num_rows($assessment_data) > 0) {
        while ($assmnt = tep_db_fetch_array($assessment_data)) {
            ...
            'invite_link' => '<a class="btn btn-success text-white" style="color:#fff!important;" href="' . tep_href_link(PATH_TO_QUIZ . FILENAME_RECRUITER_ASSESSMENT, 'id='.tep_db_output($assmnt['id']).'&action=view') . '">'.TB_HEAD_INVITE.'</a>',
            ));
        }
...
if ($action == 'new') {
    ...
    $template->assign_vars(array(
        ...
        'STEP_3'            => STEP_3,
        'STEP_4'            => STEP_4,
        'add_job'           => '<a class="text-capitalize text-primary mt-3 d-block" href="'.tep_href_link(FILENAME_RECRUITER_POST_JOB).'"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus-lg" viewBox="0 0 16 16">
                                    <path fill-rule="evenodd" d="M8 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2Z"/>
                                </svg> Add job</a>',
...
function load_custom_modal_button()
{
    ...
    <div class="row">
        <div class="col-md-4">
        <div class="card quiz-box" style="background:#e9f0f1;border-color:#e9f0f1;border-radius:0;text-align:center;">
            <div class="card-body">
