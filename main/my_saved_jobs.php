...
if($db_saved_job_num_row > 0)
{
 ...
 while ($saved_job = tep_db_fetch_array($db_saved_job_query))
 {
  ...
  $template->assign_block_vars('job', array( 'row_selected' => $row_selected,
   'job_title' => '<a href="'.tep_href_link($job_id.'/'.$title_format.'.html').'">'.tep_db_output($saved_job['job_title']).'</u></a>',
   're_adv' => date("m-d-Y", strtotime($saved_job['re_adv'])),
   'company_name' => $company_name,
   'expired' => date("m-d-Y", strtotime($saved_job['expired'])),
