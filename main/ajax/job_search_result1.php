...
if ((preg_match("/http:\/\//i",$keyword)))
 ...
 if(tep_not_null($keyword)  && (($_GET['keyword']!='keyword') && ($_GET['keyword']!='job search keywords')) )
 {
  ...
  if(tep_not_null($location) && $_GET['location']!='location')
  {
   ...
   if($map_view>=1)
  		$query = "select $field_names1 from $table_names where $whereClause ORDER BY if(j.job_source ='jobsite',0,1)  asc, j.inserted desc, j.job_featured='Yes' desc";
			else
    ...
    if($map_view<=1)
{
    ...
    if($map_view>=1)
				{
     $pages='<div class="card-footer card-footer-custom2 bg-white px-0 d-flex align-items-center">								 <div class="float-left">'.$obj->anchors.'</div>';
      ...
      $data_array[]=array('job_id'=>$ide,
       ...
       foreach($data_map_array as $key => $value)
       {
        ...
        $content.='<div id="'.$div_id.'" style="display:none">'.$div_content.'</div>';
				...
				{
     $pages='<div class="card-footer card-footer-custom2 bg-white px-0 d-flex align-items-center">'.$obj->anchors.'';
...
<div class="flex-grow-1 ms-3">
    <div class="d-flex align-items-top">
      <div class="me-2">'.(($row['post_url']=='Yes')?'':'<input class="form-check-input" type="checkbox" name="apply_job" value="'.$query_string.'">').'</div>
      <div class="d-grow-1">
      <h4>'.tep_db_output($row['job_title']).'</h4>
      <div class="text-muted small"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-building" viewBox="0 0 16 16">
          <path fill-rule="evenodd" d="M14.763.075A.5.5 0 0 1 15 .5v15a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5V14h-1v1.5a.5.5 0 0 1-.5.5h-9a.5.5 0 0 1-.5-.5V10a.5.5 0 0 1 .342-.474L6 7.64V4.5a.5.5 0 0 1 .276-.447l8-4a.5.5 0 0 1 .487.022zM6 8.694 1 10.36V15h5V8.694zM7 15h2v-1.5a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 .5.5V15h2V1.309l-7 3.5V15z"/>
          <path d="M2 11h1v1H2v-1zm2 0h1v1H4v-1zm-2 2h1v1H2v-1zm2 0h1v1H4v-1zm4-4h1v1H8V9zm2 0h1v1h-1V9zm-2 2h1v1H8v-1zm2 0h1v1h-1v-1zm2-2h1v1h-1V9zm0 2h1v1h-1v-1zM8 7h1v1H8V7zm2 0h1v1h-1V7zm2 0h1v1h-1V7zM8 5h1v1H8V5zm2 0h1v1h-1V5zm2 0h1v1h-1V5zm0-2h1v1h-1V3z"/>
        </svg> '.tep_db_output($company_name).'</div>
      <div class="text-muted small"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-geo-alt" viewBox="0 0 16 16">
          <path d="M12.166 8.94c-.524 1.062-1.234 2.12-1.96 3.07A31.493 31.493 0 0 1 8 14.58a31.481 31.481 0 0 1-2.206-2.57c-.726-.95-1.436-2.008-1.96-3.07C3.304 7.867 3 6.862 3 6a5 5 0 0 1 10 0c0 .862-.305 1.867-.834 2.94zM8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10z"/>
          <path d="M8 8a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm0 1a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
        </svg> '.tep_db_output($row['location'].' '.$row['country_name']).'</div>
      <div class="text-muted small"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-clock" viewBox="0 0 16 16">
          <path d="M8 3.5a.5.5 0 0 0-1 0V9a.5.5 0 0 0 .252.434l3.5 2a.5.5 0 0 0 .496-.868L8 8.71V3.5z"/>
          <path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16zm7-8A7 7 0 1 1 1 8a7 7 0 0 1 14 0z"/>
        </svg> '.tep_db_output($row['type_name']).'</div>
      <div class="text-muted small"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-currency-dollar" viewBox="0 0 16 16">
          <path d="M4 10.781c.148 1.667 1.513 2.85 3.591 3.003V15h1.043v-1.216c2.27-.179 3.678-1.438 3.678-3.3 0-1.59-.947-2.51-2.956-3.028l-.722-.187V3.467c1.122.11 1.879.714 2.07 1.616h1.47c-.166-1.6-1.54-2.748-3.54-2.875V1H7.591v1.233c-1.939.23-3.27 1.472-3.27 3.156 0 1.454.966 2.483 2.661 2.917l.61.162v4.031c-1.149-.17-1.94-.8-2.131-1.718H4zm3.391-3.836c-1.043-.263-1.6-.825-1.6-1.616 0-.944.704-1.641 1.8-1.828v3.495l-.2-.05zm1.591 1.872c1.287.323 1.852.859 1.852 1.769 0 1.097-.826 1.828-2.2 1.939V8.73l.348.086z"/>
        </svg> '.(tep_not_null($row['job_salary'])?$sym_left.tep_db_output($row['job_salary']).$sym_rt:'Negotiable').'</div>
      <div class="text-muted small"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-briefcase" viewBox="0 0 16 16">
          <path d="M6.5 1A1.5 1.5 0 0 0 5 2.5V3H1.5A1.5 1.5 0 0 0 0 4.5v8A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-8A1.5 1.5 0 0 0 14.5 3H11v-.5A1.5 1.5 0 0 0 9.5 1h-3zm0 1h3a.5.5 0 0 1 .5.5V3H6v-.5a.5.5 0 0 1 .5-.5zm1.886 6.914L15 7.151V12.5a.5.5 0 0 1-.5.5h-13a.5.5 0 0 1-.5-.5V7.15l6.614 1.764a1.5 1.5 0 0 0 .772 0zM1.5 4h13a.5.5 0 0 1 .5.5v1.616L8.129 7.948a.5.5 0 0 1-.258 0L1 6.116V4.5a.5.5 0 0 1 .5-.5z"/>
        </svg> '.tep_db_output(calculate_experience($row['min_experience'],$row['max_experience'])).'</div>
      ...
      <span class="result-ends">
          <span class="text-muted" style="font-size:14px;">
            Posted On: '.date("m-d-Y", strtotime($row['inserted'])).'
          </span>
      </span>
      <span class="mx-1">
          &#8226;
      ...
      <span class="result-ends for-mobile">
          <span class="text-muted" style="font-size:14px;">
            Ends On: '.date("m-d-Y", strtotime($row['expired'])).'
          </span>
      </span>
  ...
  <div class="flex-shrink-0 me-3">
      <div class="job-result-img mb-3">'.$recruiter_logo.'</div>
