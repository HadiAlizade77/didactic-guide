...
{
 if($row_gift=getAnyTableWhereData(GIFT_TABLE,'certificate_number="'.tep_db_input(tep_db_prepare_input($_POST['gift_code'])).'"',"*"))
 {
  switch($row_gift['discount_type_id'])
  {
   case 3:
    if($row_gift['amount']<$checked_price)
    {
     ...
     if($row_gift_used_1=getAnyTableWhereData(GIFT_USED_TABLE,"recruiter_id='".$_SESSION['sess_recruiterid']."' and gift_id='".tep_db_input($row_gift['gift_id'])."'","*",false))
...
if($gift_code!='')
{
 ...
 {
  if(!$row_gift_used=getAnyTableWhereData(GIFT_USED_TABLE." as gu, ".GIFT_TABLE." as g","gu.recruiter_id='".$_SESSION['sess_recruiterid']."' and g.certificate_number='".tep_db_input($gift_code)."' and gu.gift_id=g.gift_id","gu.gift_id"))
  {
