      ...
      while($row = tep_db_fetch_array($result))
      {
       ...
       $template->assign_block_vars('applicant_track', array(
          'application_id'=>$application_id,
		      ...
		      'action'=>'<a class="ddddd me-3 text-primary" href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS,'query_string3='.$query_string3.'&job_id='.$job_id.'&action1=change_status').'">'.INFO_TEXT_EDIT.'</a> <span class="ddddd text-primary" id="s_'.$ide.'" >'.(($row['applicant_select']=='Yes')?INFO_TEXT_SHORTL:'<a class="ddddd me-3 text-primary" href="'.tep_href_link(FILENAME_RECRUITER_APPLICANT_TRACKING,'query_string3='.$query_string3).'" class="select_link">'.INFO_TEXT_SELECT.'</a>').'</span> <a class="ddddd me-3 text-primary" href="'.tep_href_link(FILENAME_EMPLOYER_INTERACTION,"query_string3=".$query_string3).'" target="_blank">'.INFO_TEXT_CONTACT.'</a> <a class="ddddd me-3 text-primary" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string='.$query_string).'"><nobr>'.INFO_TEXT_VIEW_RES.'</nobr></a>',
  		    'photo'=>(tep_not_null($row['jobseeker_photo']) && is_file(PATH_TO_MAIN_PHYSICAL_PHOTO.$row['jobseeker_photo'])
                    ?tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_PHOTO.$row['jobseeker_photo'],tep_db_output(SITE_TITLE),'','','class="mini-profile-img"')
                    :defaultProfilePhotoUrl($row['jobseeker_first_name'].' '.$row['jobseeker_last_name'],false,100, 'class="mini-profile-img"')
                  ),
          'name' =>'<h4><a class="text-dark" href="'.tep_href_link(FILENAME_RECRUITER_LIST_OF_APPLICATIONS,"&search_id=".$ide).'" target="_blank">'. tep_db_output($row['jobseeker_first_name'].' '.$row['jobseeker_last_name']).'</a></h4>',
          ...
          'inserted' => date("m-d-Y", strtotime($row['inserted'])),
...
</svg> '.INFO_ALL_APP.'</a>':'<a class="btn btn-primary"  href="#" onclick="show_result(\'selected\');"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-check-fill" viewBox="0 0 16 16">
<path fill-rule="evenodd" d="M15.854 5.146a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 0 1 .708-.708L12.5 7.793l2.646-2.647a.5.5 0 0 1 .708 0z"/>
<path d="M1 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
</svg> '.INFO_ALL_SEL_APP.'</a>'),
...
/////////////////////////////////////////////////
  'SCREENING_ROUND'=>tep_draw_form('page', FILENAME_RECRUITER_APPLICANT_TRACKING,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('process_round','1')."<button type='submit' class='small-btn btn btn-outline-secondary me-2 m-border'>Screening</button></form>",
  'TELEPHONE_ROUND'=>tep_draw_form('page', FILENAME_RECRUITER_APPLICANT_TRACKING,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('process_round','3')."<button type='submit' class='small-btn btn btn-outline-secondary me-2 m-border'>Telephone</button></form>",
  'INTERVIEW_ROUND'=>tep_draw_form('page', FILENAME_RECRUITER_APPLICANT_TRACKING,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('process_round','2')."<button type='submit' class='small-btn btn btn-outline-secondary me-2 m-border'>Interview</button></form>",
  'SKILL_CHECK_ROUND'=>tep_draw_form('page', FILENAME_RECRUITER_APPLICANT_TRACKING,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('process_round','4')."<button type='submit' class='small-btn btn btn-outline-secondary me-2 m-border'>Skill Check</button></form>",
  'BACKGROUND_CHECK_ROUND'=>tep_draw_form('page', FILENAME_RECRUITER_APPLICANT_TRACKING,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('process_round','5')."<button type='submit' class='small-btn btn btn-outline-secondary me-2 m-border'>Background Check</button></form>",
  'REF_CHECK_ROUND'=>tep_draw_form('page', FILENAME_RECRUITER_APPLICANT_TRACKING,'','post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('process_round','6')."<button type='submit' class='small-btn btn btn-outline-secondary me-2 m-border'>Reference Check</button></form>",
  'ALL_ROUND_APP'=>'<a class="small-btn btn btn-outline-secondary me-2 m-border" href="">All Applicants</a>',
//////////////////////////////////////////////////
