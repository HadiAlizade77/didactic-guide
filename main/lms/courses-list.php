...
if ($action == 'course-preview' && $courseID) {
    ...
    if ($searchKeyword) {
        ...
        $search_heading = '<a href="'.tep_href_link(PATH_TO_LMS.LMS_LIST_COURSES_FILENAME).'">All Courses</a>';
    }
    ...
    if (tep_db_num_rows($query) > 0) {
        while ($row_data = tep_db_fetch_array($query)) {
            ...
            if ($row_data['course_thumbnail']) {
                ...
                $course_thumb = defaultProfilePhotoUrl($row_data['course_title'], false, 75, 'class="card-img-top"');
            }
            ...
            if (tep_db_output($row_data['course_description'])) {
                ...
                $summary = (strlen($stripTag) > 80) ? substr($stripTag,0,80).'...' : $stripTag;
            } else {
                $summary = '';
            }
...
function category_select_dropdown()
{
    ...
    $selectDiv = '<form><select name="c" class="form-select" onchange="this.form.submit()"><option value="0">Select Course</option>';
...
{
    $all_course_url = "'".tep_href_link(PATH_TO_LMS.LMS_LIST_COURSES_FILENAME)."'"; 
    $divRow = '<div class="row row-cols-1 row-cols-md-4 g-5 text-center">';
    ...
    if (count($data) > 0) {
        foreach ($data as $key => $row_data) {
            if ($row_data['category_img']) {
                $img = '<img class="py-3" src="'.tep_href_link(PATH_TO_UPLOAD_IMAGE . $row_data['category_img']).'" width="75" alt="category_img_'.$key.'">';
            }else{
                $img = defaultProfilePhotoUrl($row_data['category_name'], false, 75);
            }

            $divRow .= '<div class="col" id="cat-'.$key.'">
                            '.$img.'
                            <h4 class="card-title"><a href="'.tep_href_link(PATH_TO_LMS.LMS_LIST_COURSES_FILENAME,'c='.$row_data['category_name']).'">'.$row_data['category_name'].'</h4></a>
                        </div>';
        }
    }
