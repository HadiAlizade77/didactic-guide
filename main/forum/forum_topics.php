  ...
  case 'save':
   ...
   if(!$error)
   {
    ...
    $sql_data_array1=array('topic_id'         => $last_topic_id,
     ...
     tep_redirect(tep_href_link(PATH_TO_FORUM.$forum_id.'/'.encode_forum($forum_info['title']).'.html'));
   } 
 }
}
////////////////////////////////////////////
  ...
  ///only for sorting ends
   ...
   if($x > 0 && $x1 > 0)
   {
    ...
    while ($topic = tep_db_fetch_array($topic_query)) 
    {
     ...
     $topic_name   = "<a href='".tep_href_link(PATH_TO_FORUM.encode_forum($key).'_'.$topic['id'].'.html')."'  title='".tep_db_output($key1)."' class='1forum_title1 text-dark'>".tep_db_output($key)."</a>";
...
if($action=='preview') 
{
 ...
 $forum_header_link.='&nbsp;&gt;&gt;&nbsp;<a href="'.tep_href_link(PATH_TO_FORUM.$forum_info['id'].'/'.encode_forum($forum_info['title']).'.html').'" class="forum_sub_heading">'.tep_db_input($forum_info['title']).'</a>';
 ...
 'forum_title'   => '<a href="'.tep_href_link(PATH_TO_FORUM.$forum_id.'/'.encode_forum($forum_info['title']).'.html').'" style="font-family: Arial, Helvetica, sans-serif;font-size: 16px;font-weight: bold;">'.$forum_info['title'].'</a>',
...
'button'=>tep_button_submit('btn btn-primary','Save'),
 'INFO_TEXT_FORUM_HEADER_LINK'  => $forum_header_link,
 'forum_title'   => '<a href="'.tep_href_link(PATH_TO_FORUM.$forum_id.'/'.encode_forum($forum_info['title']).'.html').'" style="">'.$forum_info['title'].'</a>',
 'form'=>$form,
...
else
{
 ...
 $forum_header_link.='&nbsp;&gt;&gt;&nbsp;<a href="'.tep_href_link(PATH_TO_FORUM.$forum_info['id'].'/'.encode_forum($forum_info['title']).'.html').'" class="forum_sub_heading1">'.tep_db_input($forum_info['title']).'</a>';
 $template->assign_vars(array(
 'HEADING_TITLE' => '<a href="'.tep_href_link(PATH_TO_FORUM).'"  >'.HEADING_TITLE.'</a>',
 'HOST_NAME'     => HOST_NAME,
...
'INFO_TEXT_FORUM_HEADER_LINK'  => $forum_header_link,
 'INFO_TEXT_HEADING_SEARCH' => '<a class="btn btn-outline-secondary" href="'.tep_href_link(PATH_TO_FORUM.FILENAME_FORUM_SEARCH).'"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
