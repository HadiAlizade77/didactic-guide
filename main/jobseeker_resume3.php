...
{
 $query_string=encode_string("resume_id@@@".$resume_id."@@@resume");
 $add_save_button='<button class="btn btn-primary me-2" type="submit">Save & Next</button>';//tep_image_submit(PATH_TO_BUTTON.'button_save_next.gif', IMAGE_SAVE,"");
 ...
 $education_button='<button class="btn btn-primary me-2" type="submit" onclick=set_action("education_add")>Add New Education</button> <button class="btn btn-outline-secondary px-4 me-2" type="submit" onclick=set_action("education_add_next")>Save & Next</button>';//tep_image_submit(PATH_TO_BUTTON.'add_new_education.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action('education_add_next')")."&nbsp;&nbsp;&nbsp;".tep_image_submit(PATH_TO_BUTTON.'button_save_next.gif', IMAGE_SAVE_NEXT,"onclick=set_action('education_add_next')");
	...
	if($_POST['action']=="education_edit" || $_POST['action']=="education_add" || $_POST['action']=="education_add_next")
 {
  if($_POST['action']=="education_edit")
  {
   ...
   $end_month                = $end_month;
   $end_year                 = $end_year;
   $education_button     = '<button class="btn btn-outline-secondary px-4 me-2" type="submit">Update</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
   $query_string=encode_string("resume_id@@@".$resume_id."@@@resume");
...
$add_next_button = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_RESUME4."?query_string=".$query_string)."' class='btn btn-outline-secondary px-4 me-2'>".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
