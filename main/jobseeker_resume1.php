...
{
	$add_save_button='<button class="btn btn-primary" type="submit">Next</button>';//tep_image_submit(PATH_TO_BUTTON.'button_next.gif', IMAGE_NEXT);
}
else
{
	$add_save_button='<button class="btn btn-primary" type="submit">Next</button>';//tep_image_submit(PATH_TO_BUTTON.'button_next.gif', IMAGE_NEXT);
...
if($error)
{
 ...
 {
  $add_save_button     = '<button class="btn btn-primary me-2" type="submit">Update</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
  $query_string=encode_string("resume_id@@@".$_POST['resume_id']."@@@resume");
  $add_next_button     = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_RESUME2."?query_string=".$query_string)."' class='btn btn-outline-secondary'>".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
...
else
{
 ...
 if($row2=getAnyTableWhereData(JOBSEEKER_RESUME1_TABLE,"jobseeker_id='".$_SESSION['sess_jobseekerid']."' and resume_id ='".$resume_id."'",$fields))
 {
  ...
  $add_save_button          = '<button class="btn btn-primary me-2" type="submit">Update</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
  $add_next_button          = "<a class='btn btn-outline-secondary' href='".tep_href_link(FILENAME_JOBSEEKER_RESUME2."?query_string=".$query_string)."' >".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
