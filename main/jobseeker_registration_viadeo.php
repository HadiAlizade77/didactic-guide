...
include_once(PATH_TO_MAIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_JOBSEEKER_REG_VIADEO);
$template->set_filenames(array('registration' => 'jobseeker_registration_viadeo.htm','email'=>'jobseeker_registration_template.htm','fr_email'=>'fr_jobseeker_registration_template.htm'));
...
if(isset($_SESSION['access_token']))
{
 ...
 {
  $_SESSION['viadeo_error']='through viadeo jobseeker registration disable by admin.use normal way to register';
  @session_unset();
  @session_destroy();
  tep_redirect(FILENAME_JOBSEEKER_REGISTER1);
 }
 ...
 if(!tep_not_null($action))
 {
  ...
  $TR_city       = tep_db_prepare_input($viadeo_location['city']);
  ...
  $zip             =tep_db_prepare_input($viadeo_location['zipcode']);
...
$template->assign_vars(array(
 ...
 'registration_form'       => tep_draw_form('registration', FILENAME_JOBSEEKER_REG_VIADEO, '', 'post', 'onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('action','new'),
