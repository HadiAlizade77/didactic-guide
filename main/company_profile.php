  ...
  ///only for sorting ends
   ...
   if($x > 0 && $x1 > 0)
   {
    ...
    while($row = tep_db_fetch_array($result))
    {
     ...
     {
      $company_name='<a href="'.tep_href_link(FILENAME_JOBSEEKER_COMPANY_DETAILS,'query_string='.$query_string).'">'.$company_name.'ffff</a>';
     }
	$job_category_ids=get_name_from_table(JOB_JOB_CATEGORY_TABLE,'job_category_id','job_id',$ide);
     ...
     $template->assign_block_vars('result', array( 'row_selected' => $row_selected,
      ...
      're_adv' => date("m-d-Y", strtotime($row['re_adv']));//tep_date_long(tep_db_output($row['re_adv'])),
      'expired' => date("m-d-Y", strtotime($row['expired']));//tep_date_long(tep_db_output($row['expired'])),
      'apply' => '<a class=" text-capitalize" href="'.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'">'.INFO_TEXT_APPLY_NOW.'</a>',
      ));
     $alternate++;
     $lower = $lower + 1;
    }
  ...
  if($row_info=getAnyTableWhereData(RECRUITER_LOGIN_TABLE." as rl left join ".RECRUITER_TABLE." as r on ( r.recruiter_id = rl.recruiter_id)  left outer join ".COMPANY_DESCRIPTION_TABLE." as d on (rl.recruiter_id=d.recruiter_id) left outer join  ".COUNTRIES_TABLE." as c on (r.recruiter_country_id=c.id) left outer join ".ZONES_TABLE." as z on(r.recruiter_state_id=z.zone_id )" ,$whereClause1,$filed_names))
  {
   ...
   if($company_description!='')
   {
    if(strlen($company_description)>=350)
     ...
     $company_description='<div><b>Description</b><br>'.stripslashes(substr(strip_tags($company_description,"['p', 'br']"),0,350)).' <a href="'.tep_href_link('company-'.$row_info["recruiter_company_seo_name"].'/').'"  style="color:red;">more >></a>'.' </div>';
    else
...
// $whereClause="where rl.recruiter_status='Yes'";
 ...
 $hidden_fields.=tep_draw_hidden_field('sort',$sort);
 $totalpage=ceil($x1/$higher);
 $fields="r.recruiter_company_name,d.description,rl.recruiter_email_address,r.recruiter_logo,r.recruiter_company_seo_name,r.recruiter_address1,r.recruiter_city,r.recruiter_country_id,if(r.recruiter_state_id,z.zone_name,r.recruiter_state) as recruiter_state,c.country_name";
 $query = "select $fields  from ".RECRUITER_TABLE." as r left join ".RECRUITER_LOGIN_TABLE." as rl on ( r.recruiter_id = rl.recruiter_id) left outer join ".COMPANY_DESCRIPTION_TABLE." as d on (rl.recruiter_id=d.recruiter_id) left outer join ".COUNTRIES_TABLE." as c on (r.recruiter_country_id=c.id) left outer join ".ZONES_TABLE." as z on(r.recruiter_state_id=z.zone_id ) $whereClause ORDER BY  $field $order limit $lower,$higher   ";
	...
	if($x > 0 && $x1 > 0)
 {
  ...
  while($row =  tep_db_fetch_array($result))
  {
   ...
   $query_string1=encode_string("recruiter_email=".$email_id."=mail");
    ...
    $company_name="<a href='".tep_href_link('company/'.$row["recruiter_company_seo_name"].'/')."'  class='blue'><span  class='jobs-by-company-title'>".tep_db_output($row['recruiter_company_name'])."</span></a> ";
  ...
  //  company address
   ...
   // total jobs
   $com_total_jobs = count_jobs_for_company($row["recruiter_company_name"]);

   if ( $com_total_jobs > 0) {
    $jobs_total = '<a class="text-muted" href="'.tep_href_link(FILENAME_JOBSEEKER_COMPANY_DETAILS,'query_string1='.$query_string1).'"><i class="bi bi-briefcase-fill me-1"></i> '.$com_total_jobs.' Jobs</a>';
   }else{
    $jobs_total = '';
   }
 ...
 tep_db_free_result($result);
 tep_db_free_result($result1);
 $header_link='<nav aria-label="Page navigation example"><ul class="pagination pagination-sm">';
 for($i='A';$i!='AA';$i++)
 {
