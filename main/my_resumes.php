 ...
 switch($action)
 {
  ...
  //////////////////////////////
   for($j=1;$j<=6;$j++)
   {
    ...
    $i=0;
    while ($a_row = tep_db_fetch_array($result, MYSQL_ASSOC))
    {
     $field[$i]="$a_row[Field]";
     $i++;
    }
...
if($resume_query_numrows > 0)
{
 ...
 while ($resume = tep_db_fetch_array($resume_query))
 {
  ...
  $edit=$edit_form."<a href='#' onclick=\"document.edit_resume".$alternate.".submit()\">".tep_db_output(INFO_TEXT_EDIT)."</a>".$hidden_fields1;
  $delete="<a href='#' onClick=goRemove('".FILENAME_JOBSEEKER_LIST_OF_RESUMES."','resume_id','ResultDelete','$ide');return false;>".tep_db_output(INFO_TEXT_DELETE)."</a>".$hidden_fields1;
  $view=$view_form."<a href='#' onclick=\"document.view_resume".$alternate.".submit()\" >".tep_db_output(INFO_TEXT_VIEW)."</a>".$hidden_fields1;
  $duplicate=$duplicate_form."<a href='#' onclick=\"document.duplicate_resume".$alternate.".submit()\">".tep_db_output(INFO_TEXT_DUPLICATE)."</a>".$hidden_fields1;
...
$template->assign_vars(array(
 ...
 'add_new'=>"<a class='btn btn-sm btn-primary' href='".tep_href_link(FILENAME_JOBSEEKER_RESUME1)."'><svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-plus-lg' viewBox='0 0 16 16'>
 <path fill-rule='evenodd' d='M8 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2Z'/>
</svg> ".INFO_TEXT_ADD_NEW_RESUME."</a>",
