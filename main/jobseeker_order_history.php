  ...
  if($db_order_num_row > 0)
  {
   ...
   while ($order = tep_db_fetch_array($db_order_query))
   {
    ...
    //'last_updated' => tep_date_long(tep_db_output($order['orders_date_finished'])),
		   'inserted' => (tep_not_null($order['start_date'])?date("m-d-Y", strtotime($order['start_date'])):'Wait For Admin Approval'),
     'last_updated' => (tep_not_null($order['end_date'])?date("m-d-Y", strtotime($order['end_date'])):'Wait For Admin Approval'),
