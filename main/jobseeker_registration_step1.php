...
$template->set_filenames(array('registration' => 'jobseeker_registration_step1.htm','email'=>'jobseeker_registration_template.htm','fr_email'=>'fr_jobseeker_registration_template.htm'));
...
if(check_login('jobseeker') )
{
 $add_save_button=tep_draw_submit_button_field('register','Update','class="btn btn-primary"');//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
...
{
 $add_save_button=tep_draw_submit_button_field('register','Next','class="btn btn-primary"');//tep_image_submit(PATH_TO_BUTTON.'button_next.gif', IMAGE_NEXT);
...
if(!check_login('jobseeker'))
{
 $password_data='<div class="form-group">
  ...
  $resume_data='<div class="form-group">
				 ...
				 '.tep_draw_file_field("my_resume").'<span class="m-display-table small">'.INFO_TEXT_UPLOAD_RESUME_HELP.'</span>
...
if($state_error)
{
 ...
 {
  define('INFO_TEXT_STATE1',LIST_SET_DATA(ZONES_TABLE,"",'zone_name','zone_id',"zone_name",'name="state" class="form-select mb-2"',"state",'',$state_value)." ");
...
if(!check_login("jobseeker"))
{
 ...
 $social_button='<a href="'.FILENAME_FACEBOOK_APPLICATION.'" title="Sign in with Facebook"><img class="rounded-12" width="24" src="img/facebook.png" alt="Google"></a>';
 if(MODULE_GOOGLE_PLUGIN=='enable' && MODULE_GOOGLE_PLUGIN_JOBSEEKER=='enable')
 $social_button.=' <a href="'.FILENAME_GOOGLE_APPLICATION.'" title="Sign in with Google"><img class="rounded-12" width="24" src="img/google.png" alt="Google"></a>';
 if(MODULE_LINKEDIN_PLUGIN=='enable' && MODULE_LINKEDIN_PLUGIN_JOBSEEKER=='enable')
 $social_button.=' <a href="'.FILENAME_LINKEDIN_APPLICATION.'" title="Sign in with Linkedin"><img class="rounded-12" width="24" src="img/linkedin.png" alt="Google"></a>';
 if(MODULE_TWITTER_PLUGIN_JOBSEEKER=='enable' && MODULE_TWITTER_SUBMITTER_OAUTH_CONSUMER_KEY!='')
 $social_button.=' <a href="'.FILENAME_TWITTER_APPLICATION.'" title="Sign in with Twitter"><img class="rounded-12" width="24" src="img/twitter.png" alt="Google"></a>';
 ...
 $social_button='<tr>
            	      <td><table border="0" align="right" cellpadding="0" cellspacing="0">
            	        <tr>
            	          <td><div class="orloginwith2"><span class="m-none">'.INFO_P_JOIN_USING.'</span> '.$social_button.'</div></td>
...
'INFO_TEXT_ZIP1' => tep_draw_input_field('zip', $zip,'placeholder="'.INFO_P_ZIP.'" class="form-control mb-2"',false),
 ...
 'INFO_TEXT_MOBILE1' => tep_draw_input_field('mobile', $mobile,'placeholder="'.INFO_P_MOBILE.'" class="form-control mb-2"'),
// 'INFO_TEXT_COUNTRY1' => tep_get_country_list('TR_country',$TR_country),
 ...
 'INFO_TEXT_PRIVACY1'      =>'<div class="custom-control custom-radio">'.tep_draw_radio_field('privacy', '1', '', $privacy, 'id="radio_privacy1" class="form-check-input"').'&nbsp;<label class="custom-control-label small" for="radio_privacy1">'.INFO_TEXT_PRIVACY_HIDE_ALL.'</label></div>
<div class="custom-control custom-radio">'.tep_draw_radio_field('privacy', '2', '', $privacy, 'id="radio_privacy2" class="form-check-input"').'&nbsp;<label class="custom-control-label  small" for="radio_privacy2">'.INFO_TEXT_PRIVACY_HIDE_CONTACT.'</label></div>
<div class="custom-control custom-radio">'.tep_draw_radio_field('privacy', '3', '', $privacy, 'id="radio_privacy3" class="form-check-input"').'&nbsp;<label class="custom-control-label  small" for="radio_privacy3">'.INFO_TEXT_PRIVACY_HIDE_NOTHING.'</label></div>',
 'INFO_TEXT_RESUME_SEARCHEABLE1'=>'<div class="form-check me-3">'.tep_draw_radio_field('cv_searchable', 'Yes', '', $cv_searchable, 'id="radio_cv_searchable1" class="form-check-input" ').'&nbsp;<label class="custom-control-label small" for="radio_cv_searchable1" onMouseOver="this.style.color=\'#0000aa\'" onMouseOut="this.style.color=\'#000000\'">'.INFO_TEXT_YES.'</label></div>
<div class="custom-control custom-radio">'.tep_draw_radio_field('cv_searchable', 'No', '', $cv_searchable, 'id="radio_cv_searchable2" class="form-check-input"').'&nbsp;<label class="custom-control-label small" for="radio_cv_searchable2" onMouseOver="this.style.color=\'#0000aa\'" onMouseOut="this.style.color=\'#000000\'">'.INFO_TEXT_NO.'</label></div>',
 ...
 'INFO_TEXT_NEWS_LETTER1'=>'<div class="form-check">
  ...
  <label class="custom-control-label small" for="checkbox_news">'.INFO_P_SUBSCRIBE.'</label>
</div>',
'INFO_TEXT_CREATE_ACCOUNT'=>INFO_TEXT_CREATE_ACCOUNT,
'INFO_TEXT_JOIN'=>INFO_TEXT_JOIN,
 'INFO_TEXT_AGREEMENT'     => INFO_TEXT_AGREEMENT,
'ALREADY_MEMBER'=>(check_login("jobseeker")?'':' <div class="float-right small">'.INFO_P_ALREADY_MEMBER.' <a href="'.tep_href_link(FILENAME_JOBSEEKER_LOGIN).'">'.INFO_P_SIGN_IN.'</a> </div>'),
