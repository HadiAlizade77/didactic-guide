...
$searchClause.=" ( jr1.experience_year >= '".$work_experince."' and jr1.experience_year <= '".$work_experince1."' ) and ";
 ...
 $template->assign_vars(array(
  ...
  'TABLE_HEADING_APPLICANT_SELECT'=>"<a href='#' class='white' onclick=\"submit_thispage('".$obj_sort_by_clause->return_sort_array['name'][4]."','".$lower."');\"><u>".tep_image(PATH_TO_IMAGE.'icon_correct.gif',IMAGE_SELECTED).'</u>'.$obj_sort_by_clause->return_sort_array['image'][4]."</a>",
  ));
  ...
  if($x > 0 && $x1 > 0)
  {
   ...
   while ($application = tep_db_fetch_array($result))
   {
    ...
    $template->assign_block_vars('jobs', array( 'row_selected' => $row_selected,
     ...
     'check_box' => tep_draw_checkbox_field('TR_applicant_id[]',$application['application_id']),
     ...
     'photo'=>(tep_not_null($application['jobseeker_photo']) && is_file(PATH_TO_MAIN_PHYSICAL_PHOTO.$application['jobseeker_photo'])
              ?'<a class="mini-profile-img" href="#" onclick="'.js_popup(PATH_TO_PHOTO.$application['jobseeker_photo'],SITE_TITLE).'">'.tep_image(FILENAME_IMAGE."?size=400&image_name=".PATH_TO_PHOTO.$application['jobseeker_photo'],tep_db_output(SITE_TITLE)).'</a>'
 ...
 //////////////Right Round/////////////
  ...
  $heading2[]  = array('params'=>'','text'  => '<div class="list-group mt-2"><div class="card-header">'.INFO_TEXT_VIEW_CURRENT_STATUS.'</div></div>');
...
//  $contents[] = array('align' => 'center', 'text' => TEXT_INFO_APPLICANT_OPRATION);
 $contents[] = array('align' => 'left','params'=>'',  'text' => '<a class="list-group-item list-group-item-action px-3 py-1" href="'.tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string='.$query_string2).'" target="_blank"  >'.IMAGE_PROFILE.'</a>');
...
if($action1 =='compair_profile')
{
 ...
 for($j=1;$j<=2;$j++)
 {
  ...
  if($resume_id>0)
  {
   ...
   $photo=(tep_not_null($row['jobseeker_photo']) && is_file(PATH_TO_MAIN_PHYSICAL_PHOTO.$row['jobseeker_photo'])?'<a href="#" onclick="'.js_popup(PATH_TO_PHOTO.$row['jobseeker_photo'],SITE_TITLE).'">'.tep_image(FILENAME_IMAGE."?image_name=".PATH_TO_PHOTO.$row['jobseeker_photo'],tep_db_output(SITE_TITLE)).'</a>':'');
            ...
            <td valign="top" class="jobseeker'.$j.'_profile" width="50%">
             <table border="0"  class="jobseeker'.$j.'_profile"width="100%" cellspacing="2" cellpadding="2" align="left" bgcolor="#FFFFFF">
              <tr>
               ...
               <td valign="top" align="right" rowspan="6"><div class="mini-profile-img">'.$photo.'</div></td>
              </tr>
              <tr>
               <td width="130" align="right" class="black_verdana">'.INFO_TEXT_NAME.'</td>
               <td class="gray_verdana">'.tep_db_output($row['jobseeker_first_name'].' '.$row['jobseeker_middle_name'].' '.$row['jobseeker_last_name']).'</td>
              </tr>
              <tr>
...
//////////////////////////////////////////////////
 ...
 if($db_applicant_num_row > 0)
 {
  ...
  while ($application = tep_db_fetch_array($db_applicant_query))
  {
   ...
   //else
    $status_drop_down=LIST_SET_DATA(APPLICATION_STATUS_TABLE,"",'application_status','id',"priority desc,id ",'class="form-select form-select-sm" name="new_status"',"",'',$latest_status['id']);
   $template->assign_block_vars('application_set_status',array( 'row_selected' => $row_selected,
    ...
    'change_status' => tep_not_null($application['cur_status'])?tep_db_output($application['cur_status']):'---',//"<a href='".tep_href_link(FILENAME_JOBSEEKER_VIEW_RESUME,'query_string='.$query_string)."'>". tep_db_output($application['full_name'])."</a>",
    'updated'       => date("m-d-Y", strtotime($latest_status['inserted'])),
 ...
 $template->assign_vars(array(
  ...
  'button'=>'<button class="btn btn-sm btn-primary" type="submit">Update</button>',//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE),
  'LEFT_BOX_WIDTH'=>LEFT_BOX_WIDTH1,
  'RIGHT_BOX_WIDTH'=>$RIGHT_BOX_WIDTH,
  'LEFT_HTML'=>'',
  'RIGHT_HTML'=>$RIGHT_HTML,
...
elseif($action1=='search_applicant')
{
 ...
 'INFO_TEXT_TRACKING_ROUND1' => LIST_SET_DATA(SELECTION_ROUND_TABLE,"",TEXT_LANGUAGE.'round_name','id',"value",'name="process_round" class="form-select form-select-sm"',INFO_TEXT_ALL,'',$process_round),
 'INFO_TEXT_APPLICATION_STATUS1' => LIST_SET_DATA(APPLICATION_STATUS_TABLE,"",'application_status','id',"priority",'name="application_status" class="form-select form-select-sm" ',INFO_TEXT_ALL,'',$application_status),
 ...
 'INFO_TEXT_EMAIL_ADDRESS1'=> tep_draw_input_field('TNEF_email_address',$email_address, 'class="form-control" placeholder="Email address"'),
  ...
  'INFO_TEXT_EXPERIENCE1'  => '<select name="experience" class="form-select form-select form-select-sm">
...
else
{
 ...
 $check_link1='
        ...
        <!--<label for="validationCustom01">'.INFO_TEXT_WITH_SELECTED.'</label>-->
				<select name="select_action"  class="form-select form-select-sm" onchange="select_action2();">
 ...
 $template->assign_vars(array(
  ...
  "INFO_TEXT_APPLICATION_STATUS1" => LIST_SET_DATA(APPLICATION_STATUS_TABLE,"",'application_status','id',"priority",'name="application_status" class="form-select form-select-sm" onchange="document.page.submit()"',INFO_TEXT_ALL,'',$application_status),
  "INFO_TEXT_TRACKING_ROUND" => INFO_TEXT_TRACKING_ROUND,
  "INFO_TEXT_TRACKING_ROUND1" => LIST_SET_DATA(SELECTION_ROUND_TABLE,"",TEXT_LANGUAGE.'round_name','id',"value",'name="process_round" class="form-select form-select-sm" onchange="document.page.submit()"',INFO_TEXT_ALL,'',$process_round),
  'new_button'=>'',
