...
else
{
 /*********************addition of photo*******************/
   ...
   $vedio   = "<a class='mt-2 d-inline-block img-thumbnail' href='#' onclick=\"popUp('".tep_href_link(FILENAME_DISPLAY_VIDEO,'query_string1='.$query_string)."')\" >".$video_img."</a>";
		...
		{
   $upload_button='<button class="btn btn-primary " type="submit">Update</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
		}
		else
		{
  	$upload_button='<button class="btn btn-primary " type="submit">Next</button>';//tep_image_submit(PATH_TO_BUTTON.'button_next.gif', IMAGE_NEXT);
		}
	}
}
 /*********************************************************/
$query_string       = encode_string("resume_id@@@".$resume_id."@@@resume");
$add_next_button2    = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_LIST_OF_RESUMES)."' class='btn btn-skip-next'>Skip The Step</a>";
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_ADD_VIDEO1'            => tep_draw_input_field("jobseeker_video",$jobseeker_video,'class="form-control" size="60"',false).$vedio,
