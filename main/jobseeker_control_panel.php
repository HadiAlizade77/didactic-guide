...
if(tep_not_null($resume_photo_check['jobseeker_photo']) && is_file(PATH_TO_MAIN_PHYSICAL_PHOTO.$resume_photo_check['jobseeker_photo']))
{
 ...
 $photo='<div class="no-pic mx-auto mb-2">'.$photo.'</div>
 <a href="'.tep_href_link(FILENAME_JOBSEEKER_RESUME5,'query_string='.$query_string).'" class="mt-2 small text-muted d-block"><i class="fa fa-pencil" aria-hidden="true"></i> '.INFO_TEXT_EDIT_PHOTO.'</a>';
}
else
{
...
if(strlen($row['job_title']) > 30)
 ...
 $title=' <a class="job_search_title" href="'.tep_href_link($ide.'/'.$title_format.'.html').'" target="_blank" class="job_search_title">'.$name_short.'</a>';


$job_posted = date("m-d-Y", strtotime($row['re_adv']));//tep_date_long(tep_db_output($row['re_adv']));
...
'JOB_ALERT_BOX'=>(JOBSEEKER_MEMBERSHIP=='false'?'':'
						<div class="col-md-12">
                            <div class="">
                                <div class="">
                                    <div class="mb-2">
                                        Get your Resume featured in Search Resume
                                    </div>
                                    <a class="d-block" href="'.tep_href_link(FILENAME_JOBSEEKER_RATES).'"><i class="bi bi-cart"></i> Rate Card</a>
                                </div>
                            </div>
                        </div>'),
'JOB_ALERT_BOX5'=>(JOBSEEKER_MEMBERSHIP=='false'?'':'<a class="" href="'.tep_href_link(FILENAME_JOBSEEKER_RATES).'"><i class="fa fa-shopping-cart" aria-hidden="true">
  ...
  'INFO_TEXT_INTERVIEW'    =>'<a href="'.tep_href_link(FILENAME_JOBSEEKER_MAILS).'" class="style39">Interview</a>',
   'INFO_TEXT_RATECARD'    =>'<a href="'.tep_href_link(FILENAME_JOBSEEKER_RATES).'" class="style39">Rate Card</a>',

    'INFO_LMS'=>((LMS_SETTING=='True')?'<a href="'. tep_href_link(PATH_TO_LMS.LMS_MY_COURSES_FILENAME) . '">My Courses</a>':''),

    'INFO_TEST'=>((TEST_SETTING=='True')?'<div class="pb-1"><a class="style39" href="'.tep_href_link(PATH_TO_QUIZ.FILENAME_JOBSEEKER_TEST_REPORT).'">My Tests</a></div>':''),

	'SHOW_ALL_JOBS'=>tep_draw_form('all_jobs', FILENAME_JOB_SEARCH,'','post').tep_draw_hidden_field('action','search').'<div class="mt-3 mb-4 text-center"><a class="text-muted" href="" onclick="document.all_jobs.submit()">Show all</a></div></form>',

	'MY_APPLICATIONS'=>'<a href="'.tep_href_link(FILENAME_JOBSEEKER_LIST_OF_APPLICATIONS).'" class="ats-btn mx-auto"><i class="bi bi-person-video me-2" style="font-size: 24px;"></i> My Applications</a>',
