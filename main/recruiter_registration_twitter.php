...
include_once(PATH_TO_MAIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_RECRUITER_REG_TWITTER);
$template->set_filenames(array('registration' => 'recruiter_registration_twitter.htm','email'=>'recruiter_registration_template.htm','fr_email'=>'fr_recruiter_registration_template.htm'));
$jscript_file=PATH_TO_LANGUAGE.$language."/jscript/".'recruiter_registation.js';
include_once(FILENAME_BODY);
//define('CONSUMER_KEY','5Q9UqZ21xNUT5fYqHBBw');
...
if(MODULE_TWITTER_PLUGIN_RECRUITER!='enable')
{
 ...
 $messageStack->add_session('through twitter recruiter registration disable by admin.use normal way to register','error');
 tep_redirect(FILENAME_RECRUITER_REGISTRATION);
}
...
if(isset($_SESSION['access_token']))
{
 ...
 if($row_linked=getAnyTableWhereData(TWITTER_USER_TABLE,"twitter_id='".tep_db_input($twitter_id)."'","user_type,user_id"))
 {
  tep_redirect(tep_href_link(FILENAME_TWITTER_APPLICATION,'request=login_info&user_type='.$row_linked['user_type']));
 }
 if(MODULE_TWITTER_PLUGIN_RECRUITER!='enable')
 {
  $messageStack->add_session('through twitter recruiter registration disable by admin.use normal way to register','error');
 ...
 if(!tep_not_null($action))
 {
  ...
  $city          = tep_db_prepare_input($content->location);
 }
}
else
{
 tep_redirect(tep_href_link(FILENAME_TWITTER_APPLICATION,'user_type=recruiter'));
}
//print_r($_POST);
//////////////////////////////
// add/edit
...
//////////////////////////////
{
 ...
 $registration_form=tep_draw_form('registration', FILENAME_RECRUITER_REG_TWITTER, '', 'post', 'enctype="multipart/form-data" onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('action','new');
}
if($error)
{
