...
$last_modified=date("m-d-Y", strtotime($row['re_adv']));
else
$last_modified=date("m-d-Y", strtotime($row['updated']));
if($row['post_url']=='Yes')
{
...
$button_apply='<input type="button" name="apply" value="Login to apply" class="btn btn-primary mb-2 mr-2 m-btn-block" onclick="location.href=\''.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'\'">
						<input type="button" name="apply" value="Register and apply" class="btn btn-outline-secondary mb-2 m-btn-block" onclick="location.href=\''.tep_href_link(FILENAME_JOBSEEKER_REGISTER1,'job='.$query_string).'\'">'.($row['recruiter_applywithoutlogin']=='Yes'?
...
if(tep_not_null($row['indeed_id']))
{
 $template->assign_vars(array(
                               ...
                               'INFO_TEXT_JOB_DETAILS1'=>nl2br(stripslashes($row['job_short_description'])).'<div align="right" ><a href="'.$row['indeed_url'].'" target="_blank" class="red">More >></a></div><br>',
                                ...
                                'button'=>'<span id="job_detail_back"><a href="javascript:history.back();">'.INFO_TEXT_BACK.'</a>&nbsp;|&nbsp;</span><a href="'.tep_href_link($job_id.'/'.$title_format.'.html','action=save').'">'.INFO_TEXT_SAVE.'</a>&nbsp;|&nbsp;<a href="'.tep_href_link(FILENAME_TELL_TO_FRIEND,'query_string='.$query_string).'">'.INFO_TEXT_TELL_TO_FRIEND.'</a>',
                              ));
}
...
else
{
 $template->assign_vars(array(
   ...
   'button'=>'<a class="btn btn-outline-secondary" href="'.tep_href_link($job_id.'/'.$title_format.'.html','action=save').'"><svg xmlns="http://www.w3.org/2000/svg" title="Save this job" width="16" height="16" fill="currentColor" class="bi bi-save" viewBox="0 0 16 16">
    <path d="M2 1a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H9.5a1 1 0 0 0-1 1v7.293l2.646-2.647a.5.5 0 0 1 .708.708l-3.5 3.5a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L7.5 9.293V2a2 2 0 0 1 2-2H14a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h2.5a.5.5 0 0 1 0 1H2z"/>
  </svg></a>

   <a class="btn btn-outline-secondary" href="'.tep_href_link(FILENAME_TELL_TO_FRIEND,'query_string='.$query_string).'"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-share" viewBox="0 0 16 16">
   <path d="M13.5 1a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3zM11 2.5a2.5 2.5 0 1 1 .603 1.628l-6.718 3.12a2.499 2.499 0 0 1 0 1.504l6.718 3.12a2.5 2.5 0 1 1-.488.876l-6.718-3.12a2.5 2.5 0 1 1 0-3.256l6.718-3.12A2.5 2.5 0 0 1 11 2.5zm-8.5 4a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3zm11 5.5a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3z"/>
 </svg></a>

   <a class="btn btn-outline-secondary" href="#" onclick="popUp(\''.tep_href_link($job_id.'/'.$title_format.'.html','action=print').'\')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-printer" viewBox="0 0 16 16">
...
'form_nologin'=>tep_draw_form('apply_nologin', FILENAME_JOB_DETAILS,'','post'),
 ...
 'company_logo'=>$company_logo,
  ...
  'advance_search'=>'<a href="'.tep_href_link(FILENAME_JOB_SEARCH).'">Advanced search</a>',
   'jobs_by_category'=>'<a href="'.tep_href_link(FILENAME_JOB_SEARCH_BY_INDUSTRY).'" title="Jobs by Category">'.INFO_TEXT_L_BY_CATEGORY.'</a>',
   'jobs_by_companies'=> '<a href="'.tep_href_link(FILENAME_JOBSEEKER_COMPANY_PROFILE).'" title="'.INFO_TEXT_L_BY_COMPANY.'">'.INFO_TEXT_L_BY_COMPANY.'</a>',//.(($no_of_companies>0)?"(".$no_of_companies.")":''),
  'jobs_by_location'=>'<a href="'.tep_href_link(FILENAME_JOB_SEARCH_BY_LOCATION).'"  title="'.INFO_TEXT_L_BY_LOCATION.'">'.INFO_TEXT_L_BY_LOCATION.'</a>',
 ...
 'INFO_TEXT_COMPANY_NAME1'=>'<a href="'.tep_href_link(FILENAME_JOBSEEKER_COMPANY_DETAILS,'query_string='.$query_string).'" title="'.INFO_TEXT_VIEW_COMPANY_PROFILE.'">'.$recruiter_company_name.'</a>',
 ...
 'INFO_TEXT_JOB_POSTED1'=>date("m-d-Y", strtotime($row['re_adv'])),
 ...
 'INFO_TEXT_JOB_APPLY_BEFORE1'=>date("m-d-Y", strtotime($row['expired'])),
 ...
 'INFO_TEXT_JOB_RESP1'=>nl2br(stripslashes($row['job_responsibility'])),
 'INFO_TEXT_JOB_QUAL'=>INFO_TEXT_JOB_QUAL,
 'INFO_TEXT_JOB_QUAL1'=>nl2br(stripslashes($row['job_qualification'])),
 'INFO_TEXT_JOB_BENEFIT'=>INFO_TEXT_JOB_BENEFIT,
 'INFO_TEXT_JOB_BENEFIT1'=>nl2br(stripslashes($row['job_benefits'])),
 'INFO_TEXT_JOB_SCHEDULE'=>INFO_TEXT_JOB_SCHEDULE,
 'INFO_TEXT_JOB_SCHEDULE1'=>$row['job_schedule'],
...
// 'view_company_profile'=>tep_draw_form('search',FILENAME_JOBSEEKER_COMPANY_PROFILE,'', 'post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('recruiter_id',tep_db_get_field(JOB_TABLE,'recruiter_id','job_id='.$job_id)).'<a href="#" onclick="submit()" title="View company profile">View company profile</a></form>',
 ...
 'INFO_TEXT_SIMILAR_JOBS'=>tep_draw_form('search_smililar_job',FILENAME_JOB_SEARCH,'', 'post').tep_draw_hidden_field('action','search').tep_draw_hidden_field('country',$row['job_country_id']).tep_draw_hidden_field('job_category[]',(tep_db_output($job_category_ids)!='0' && $job_category_ids!='')?$job_category_ids:'0').'<a class="btn btn-outline-secondary addMe" href="#" onclick="document.search_smililar_job.submit()" title="'.INFO_TEXT_VIEW_SIMILAR_JOBS.'" ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-briefcase" viewBox="0 0 16 16">
  <path d="M6.5 1A1.5 1.5 0 0 0 5 2.5V3H1.5A1.5 1.5 0 0 0 0 4.5v8A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-8A1.5 1.5 0 0 0 14.5 3H11v-.5A1.5 1.5 0 0 0 9.5 1h-3zm0 1h3a.5.5 0 0 1 .5.5V3H6v-.5a.5.5 0 0 1 .5-.5zm1.886 6.914L15 7.151V12.5a.5.5 0 0 1-.5.5h-13a.5.5 0 0 1-.5-.5V7.15l6.614 1.764a1.5 1.5 0 0 0 .772 0zM1.5 4h13a.5.5 0 0 1 .5.5v1.616L8.129 7.948a.5.5 0 0 1-.258 0L1 6.116V4.5a.5.5 0 0 1 .5-.5z"/>
</svg> <span class="for-mobile"> '.INFO_TEXT_SIMILAR_JOBS.'</span></a></form>',
