...
include_once(PATH_TO_MAIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_JOBSEEKER_REG_FACEBOOK);
$template->set_filenames(array('registration' => 'jobseeker_registration_facebook.htm','email'=>'jobseeker_registration_template.htm','fr_email'=>'fr_jobseeker_registration_template.htm'));
...
if(isset($_SESSION['access_token']))
{
 ...
 $content = $connection->get('https://graph.facebook.com/v2.8/me?fields=id,name,first_name,middle_name,last_name,email,location&access_token='.$token);
 if(!tep_not_null($content['email']))
 {
  ...
  tep_redirect('https://www.facebook.com/logout.php?next='.urlencode(tep_href_link(FILENAME_JOBSEEKER_REGISTER1)).'&access_token='.$token);
 }
 ...
 if(MODULE_FACEBOOK_PLUGIN_JOBSEEKER!='enable')
 {
  ...
  $messageStack->add_session('through Facebook jobseeker registration disable by admin.use normal way to register','error');
  unset($_SESSION['access_token']);
  tep_redirect('https://www.facebook.com/logout.php?next='.urlencode(tep_href_link(FILENAME_JOBSEEKER_REGISTER1)).'&access_token='.$token);
 }
 if(!tep_not_null($action))
 {
  ...
  $middle_name   = tep_db_prepare_input($content['middle_name']);
  $TR_city       = tep_db_prepare_input($content['location']['name']);
 }
 ...
 switch($action)
 {
  case 'new':
   ...
   if(!$error)
   {
    ...
    if(!check_login('jobseeker') && $email_address!='')
    {
     $password= randomize();
     ...
     unset($_SESSION['access_token']);
     tep_redirect('https://www.facebook.com/logout.php?next='.urlencode(tep_href_link(FILENAME_JOBSEEKER_REGISTRATION_CONFIRM)).'&access_token='.$token);
   	}
   }
  break;
 }
...
$template->assign_vars(array(
 ...
 'registration_form'       => tep_draw_form('registration', FILENAME_JOBSEEKER_REG_FACEBOOK, '', 'post', 'onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('action','new'),
