...
if($action=="send")
{
 ...
 if(!$error)
 {
  ...
  tep_redirect($job_id.'/'.$title_format.'.html');
 }
}
else
{
 $TR_message="\n\n\n\n\n\n\n\n\n".INFO_TEXT_CHECK_LINK."\n ".INFO_TEXT_JOB_TITLE." <a href=".tep_href_link($job_id.'/'.$title_format.'.html')." target='_job_detail'>".tep_db_output($row['job_title'])."</a>";
}
