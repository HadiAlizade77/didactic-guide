...
if($edit)
{
 ...
 $description=$row_check['cover_letter'];
	...
	$add_save_button.="&nbsp;<a href='".tep_href_link(FILENAME_JOBSEEKER_LIST_OF_COVER_LETTERS)."' type='button' class='btn btn-outline-secondary mx-2 m-d-block'>Cancel</a>";
...
if($db_cover_letter_num_row > 0)
{
 ...
 while ($cover_letter = tep_db_fetch_array($db_cover_letter_query))
 {
  ...
  $template->assign_block_vars('cover_letter', array( 'row_selected' => $row_selected,
   ...
   'inserted' => date("m-d-Y", strtotime($cover_letter['inserted'])),
   'updated' => date("m-d-Y", strtotime($cover_letter['updated'])),
...
else
{
 ...
 $template->assign_vars(array(
  ...
  'add_new'=>"<a class='btn btn-primary' href='".tep_href_link(FILENAME_JOBSEEKER_LIST_OF_COVER_LETTERS, 'action=add_new')."' ><svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-plus-lg' viewBox='0 0 16 16'>
