...
{
 $query_string=encode_string("resume_id@@@".$resume_id."@@@resume");
 $reference_button='<button class="btn btn-primary mr-2" type="submit" onclick=set_action1("reference_add")>Add New Reference</button> <button class="btn btn-outline-secondary px-4 me-2" type="submit" onclick=set_action1("reference_add_next")>Save & Next</button>';//tep_image_submit(PATH_TO_BUTTON.'add_new_reference.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action1('reference_add')")."".tep_image_submit(PATH_TO_BUTTON.'button_save_next.gif', IMAGE_SAVE_NEXT,"onclick=set_action1('reference_add_next')");
 ...
 if($_POST['action']=="reference_edit" || $_POST['action']=="reference_add" || $_POST['action']=="reference_add_next")
 {
  if($_POST['action']=="reference_edit")
  {
   ...
   $refname            = $refname;
   $company_name    = $company_name;
   $reference_button= '<button class="btn btn-outline-secondary px-4 me-2" type="submit">Update</button>';//tep_image_submit(PATH_TO_BUTTON.'button_update.gif', IMAGE_UPDATE);
   $query_string    = encode_string("resume_id@@@".$resume_id."@@@resume");
			$add_next_button     = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_RESUME3."?query_string=".$query_string)."' class='btn btn-outline-secondary px-4 me-2'>".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
  ...
  {
   $reference_button='<button class="btn btn-primary me-2" type="submit" onclick=set_action1("reference_add")>Add New Reference</button> <button class="btn btn-outline-secondary px-4 me-2" type="submit" onclick=set_action1("reference_add_next")>Save & Next</button>';//tep_image_submit(PATH_TO_BUTTON.'add_new_reference.gif', IMAGE_SAVE_ADD_NEW," onclick=set_action1('reference_add')")."".tep_image_submit(PATH_TO_BUTTON.'button_save_next.gif', IMAGE_SAVE_NEXT,"onclick=set_action1('reference_add_next')");
   $query_string=encode_string("resume_id@@@".$resume_id."@@@resume");
			$add_next_button     = "<a  href='".tep_href_link(FILENAME_JOBSEEKER_RESUME3."?query_string=".$query_string)."' class='btn btn-outline-secondary px-4 me-2'>".tep_db_output(INFO_SKIP_THIS_STEP)."</a>";
