  ...
  while ($rows = tep_db_fetch_array($query_result)) 
  {
   ...
   switch($rows['configuration_name'])
   {
    ...
    case 'SITE_ARTICLE_CATEGORIES':
     ...
     {
      $site_map[]=array('title'=>tep_db_output($category_list[$i]['name']),'link'=>tep_href_link('article_cat_'.$category_list[$i]['id'].'.html'),'lable'=>$category_list[$i]['lable']);
     }
     break;
    case 'SITE_JOB_CATEGORIES':
     ...
     for($i=0;$i<$total_catgories;$i++)
     {
      $site_map[]=array('title'=>tep_db_output($category_list[$i]['name']),'link'=>tep_href_link($category_list[$i]['id'].'/'.encode_category($category_list[$i]['name']).'-jobs.html'),'lable'=>$category_list[$i]['lable']);
     }
     break;
    ...
    case 'SITE_ACTIVE_JOB':
     ...
     for($c=0;$c<$total_jobs;$c++)
     {
      ...
      while ($rows = tep_db_fetch_array($query_result1)) 
      {
       ...
       $site_map[]=array('title'=>tep_db_output($rows['job_title']),'link'=>tep_href_link($rows['job_id'].'/'.$title_format.'.html'),'lable'=>1);       
      }
    ...
    case 'SITEMAP_FORUM_TOPICS':
     ...
     for($c=0;$c<$total_jobs;$c++)
     {
      ...
      while ($rows = tep_db_fetch_array($query_result1)) 
      {
       ...
       $site_map[]=array('title'=>tep_db_output($rows['title']),'link'=>tep_href_link(PATH_TO_FORUM.$rows['id'].'/'.$title_format.'.html'),'lable'=>1);       
      }
      tep_db_free_result($query_result1);
     }
     break;
...
{
 $query="select c.id, c.category_name  from ".JOB_CATEGORY_TABLE."  as c  where c.sub_cat_id  is  null order by category_name asc";
 $query_result = tep_db_query($query);
 $total_record = tep_db_num_rows($query_result);
 $category_array=array();
 if($total_record>0)
