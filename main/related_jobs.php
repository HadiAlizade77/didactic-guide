...
while($row = tep_db_fetch_array($result))
{
 ...
 $title='<a href="'.tep_href_link($ide.'/'.$title_format.'.html').'"  title="'.tep_db_output($row['job_title']).'"target="_blank">'.tep_db_output($name_short).'</a>';
 ...
 if(tep_not_null($company_logo) && is_file(PATH_TO_MAIN_PHYSICAL.PATH_TO_LOGO.$company_logo))
 {
  ...
  $apply_job    .='<a class="btn btn-block btn-sm btn-primary" href="'.$post_url.'" target="_blank">'.INFO_TEXT_APPLY_TO_THIS_JOB.'</a>';
 }
	else
	$apply_job    .='<a class="btn btn-block btn-sm btn-primary" href="'.tep_href_link(FILENAME_APPLY_NOW,'query_string='.$query_string).'"  target="_blank">'.INFO_TEXT_APPLY_TO_THIS_JOB.'</a>';
