...
include_once(PATH_TO_MAIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_RECRUITER_REG_LINKEDIN);
$template->set_filenames(array('registration' => 'recruiter_registration_linkedin.htm','email'=>'recruiter_registration_template.htm','fr_email'=>'fr_recruiter_registration_template.htm'));
...
if(isset($_SESSION['access_token']))
{
 ...
 {
  tep_redirect(tep_href_link(FILENAME_LINKEDIN_APPLICATION,'request=recruiter_info'));
 }
 elseif($row_info=getAnyTableWhereData(RECRUITER_USERS_TABLE," email_address='".tep_db_input($email_address)."'",'id'))
 {
  tep_redirect(tep_href_link(FILENAME_LINKEDIN_APPLICATION,'request=recruiter_info'));
 }
 ...
 {
  $_SESSION['REDIRECT_URL']=FILENAME_RECRUITER_REGISTRATION;
  $_SESSION['linkedin_error']='through linkedin recruiter registration disable by admin.use normal way to register';
  unset($_SESSION['access_token']);
  tep_redirect(FILENAME_LINKEDIN_APPLICATION1);
 }
 if(!tep_not_null($action))
 {
  $r=$connection->getProfile();
  $TR_first_name = tep_db_prepare_input($r['localizedFirstName']);
  $TR_last_name  = tep_db_prepare_input($r['localizedLastName']);
  $city          ='';
  $TR_telephone_number='';
  $TR_address_line1='';
  $TR_country='';
...
{
 tep_redirect(tep_href_link(FILENAME_LINKEDIN_APPLICATION,'user_type=recruiter'));
}
//print_r($_POST);
//////////////////////////////
// add/edit
  ...
  case 'new':
   ...
   if(!$error)
   {
    ...
    if(!check_login('recruiter'))
    {
     ...
     $sql_data_array2=array('linkedin_id'=>$linkedin_id,
                           'user_type'=>'recruiter',
                           'user_id'=>$_SESSION['sess_recruiterid'],
                           );
     tep_db_perform(LINKEDIN_USER_TABLE,$sql_data_array2);
     ...
     unset($_SESSION['access_token']);
     $_SESSION['REDIRECT_URL']=FILENAME_RECRUITER_REGISTRATION_CONFIRM;
     tep_redirect(tep_href_link(FILENAME_LINKEDIN_APPLICATION1));
    }
   }
  break;
 }
...
//////////////////////////////
{
 ...
 $registration_form=tep_draw_form('registration', FILENAME_RECRUITER_REG_LINKEDIN, '', 'post', 'enctype="multipart/form-data" onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('action','new');
}
if($error)
{
...
$template->assign_vars(array(
 ...
 'INFO_TEXT_EMAIL_ADDRESS1'=>$TREF_email_address,//tep_draw_input_field('TREF_email_address', $TREF_email_address,'size="50" maxlength="50"',true),
