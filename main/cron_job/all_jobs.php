...
///*
 ...
 $now=date('Y-m-d H:i:s');
	...
	$query = "select $field_names from $table_names where $whereClause ORDER BY re_adv desc limit 0,30";
 ...
 while($row = tep_db_fetch_array($result))
 {
  ...
  $file_content.= '<link>'.tep_href_link($ide.'/'.$title_format.'.html').'</link>'."\n";
  $file_content.= '<guid>'.tep_href_link($ide.'/'.$title_format.'.html').'</guid>'."\n";
  $file_content.= '</item>'."\n" ;
 //*/
 }
 $file_content.= '</channel>'."\n"  ;
