...
$whereClauses="sr.jobseeker_id=jl.jobseeker_id and sr.jobseeker_id=j.jobseeker_id and sr.job_alert='weekly'";
...
while($row=tep_db_fetch_array($result))
{
 ...
 if(tep_not_null($Industry_Sector) )
 {
  ...
  if($Industry_Sector[0]!=0)
  {
   ...
   $now=date('Y-m-d');
   $prev_day=date("Y-m-d",mktime(0,0,0, date("m"), date("d")-7, date("Y")));
