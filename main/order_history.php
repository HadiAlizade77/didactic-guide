...
include_once(PATH_TO_MAIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_RECRUITER_ACCOUNT_HISTORY_INFO);
$template->set_filenames(array('order' => 'order_history.htm','invoice' => 'invoice.htm'));
...
if(tep_not_null($order_id))
{
 ...
 if(!$row_order_check=getAnyTableWhereData(ORDER_TABLE,"orders_id='".tep_db_input($order_id)."' and recruiter_id='".$_SESSION['sess_recruiterid']."'","orders_id,admin_comment"))
 {
 ...
 $customer_address=$order->customer['company']."<br>".
                   ...
                   $order->customer['telephone']."<br>".INFO_TEXT_EMAIL_ADDRESS."<a href='mailto:".$order->customer['email_address']."'>".
                   ...
                   $order->billing['zip']."<br> ".INFO_TEXT_PHONE.
 ...
 for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++)
 {
  $order_total_text.='
	...
	$template->assign_vars(array(
		...
		'HEADING_TITLE1'=>'<a href="' . tep_href_link(FILENAME_RECRUITER_ACCOUNT_HISTORY_INFO). '"><button class="btn btn-primary">Back</button></a>',
		...
		'form'=>tep_draw_form('status', PATH_TO_ADMIN.FILENAME_ADMIN1_RECRUITER_ORDERS, tep_get_all_get_params(array('action')) . 'action=update_order'),
		'RIGHT_BOX_WIDTH'=>$RIGHT_BOX_WIDTH,
		'ADMIN_RIGHT_HTML'=>$ADMIN_RIGHT_HTML,
		'PRINT_INVOICE' => tep_link_button_Name(
			tep_href_link(FILENAME_ADMIN1_PDF_ORDERS, 'action=invoice&inv_id='.$order_id.'&q=recruiter'),
...
else
{
 ...
 define('MAX_DISPLAY_LIST_OF_ORDERS','20');
  $db_order_query_raw = "select oh.*,o.orders_date_finished, os.orders_status_name from " . ORDER_HISTORY_TABLE . " as oh, ".ORDER_TABLE." as o right join ".ORDER_STATUS_TABLE." as os on (o.orders_status=os.orders_status_id and os.language_id='".(int)$languages_id."')  where oh.order_id=o.orders_id and o.recruiter_id='".$_SESSION['sess_recruiterid']."' order by ".$order_by_clause;
  ...
  if($db_order_num_row > 0)
  {
   ...
   {
/////////////////////*******status colour********///////////////////
if(strtolower($order['orders_status_name']=='pending'))
	$statusclass='class="text-danger-custom"';
elseif(strtolower($order['orders_status_name']=='completed'))
	$statusclass='class="text-success-custom"';
elseif(strtolower($order['orders_status_name']=='processing'))
	$statusclass='class="text-dark-custom"';
else
	$statusclass='';
/////////////////////////////////////////////////////////////////////////
    $row_selected=' class="dataTableRow'.($alternate%2==1?'1':'2').'" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_RECRUITER_ACCOUNT_HISTORY_INFO,'order_id='.$order['order_id']). '\'"';
    $template->assign_block_vars('orders', array( 'row_selected' => $row_selected,
     ...
     'status' => tep_db_output($order['orders_status_name']),//'<span '.$statusclass.'>'.tep_db_output($order['orders_status_name']).'</span>',
     'inserted' => date("m-d-Y", strtotime($order['inserted'])),
     'last_updated' => date("m-d-Y", strtotime($order['orders_date_finished'])),
     ));
    $alternate++;
   }
  }
 ...
 /////
  ...
  $template->assign_vars(array(
   'HEADING_TITLE'=>HEADING_TITLE,
   'TABLE_HEADING_PLAN_TYPE_NAME'=>"<a href='".tep_href_link(FILENAME_RECRUITER_ACCOUNT_HISTORY_INFO, tep_get_all_get_params(array('sort','action')))."&sort=".$obj_sort_by_clause->return_sort_array['name'][0]."' class='white'>".TABLE_HEADING_PLAN_TYPE_NAME.$obj_sort_by_clause->return_sort_array['image'][0]."</a>",
   'TABLE_HEADING_PRICE'=>TABLE_HEADING_PRICE,
   'TABLE_HEADING_PLAN_TYPE_TIME_PERIOD'=>TABLE_HEADING_PLAN_TYPE_TIME_PERIOD,
   'TABLE_HEADING_STATUS'=>TABLE_HEADING_STATUS,
   'TABLE_HEADING_INSERTED'=>"<a href='".tep_href_link(FILENAME_RECRUITER_ACCOUNT_HISTORY_INFO, tep_get_all_get_params(array('sort','action')))."&sort=".$obj_sort_by_clause->return_sort_array['name'][1]."' class='white'>".TABLE_HEADING_INSERTED.$obj_sort_by_clause->return_sort_array['image'][1]."</a>",
