...
include_once(PATH_TO_MAIN_PHYSICAL_LANGUAGE.$language.'/'.FILENAME_JOBSEEKER_REG_LINKEDIN);
$template->set_filenames(array('registration' => 'jobseeker_registration_linkedin.htm','email'=>'jobseeker_registration_template.htm','fr_email'=>'fr_jobseeker_registration_template.htm'));
...
if(isset($_SESSION['access_token']))
{
 ...
 {
  $_SESSION['REDIRECT_URL']=FILENAME_JOBSEEKER_REGISTER1;
  $_SESSION['linkedin_error']='through linkedin jobseeker registration disable by admin.use normal way to register';
  unset($_SESSION['access_token']);
  ...
  case 'new':
   ...
   if(!$error)
   {
    ...
    if(!check_login('jobseeker'))
    {
     ...
     unset($_SESSION['access_token']);
     $_SESSION['REDIRECT_URL']=FILENAME_JOBSEEKER_REGISTRATION_CONFIRM;
...
$template->assign_vars(array(
 ...
 'registration_form'       => tep_draw_form('registration', FILENAME_JOBSEEKER_REG_LINKEDIN, '', 'post', 'onsubmit="return ValidateForm(this)"').tep_draw_hidden_field('action','new'),
...
// 'INFO_TEXT_MOBILE1'       => countty_code_phone_no('name="mobile_no"', 'please select ...', '0', $mobile_phone),
 ...
 'INFO_TEXT_EMAIL_ADDRESS1'=> $email_address ,//tep_draw_input_field('TREF_email_address', $TREF_email_address,'size="30"',true),
