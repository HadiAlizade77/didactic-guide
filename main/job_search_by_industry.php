...
//////////////////////////////////////////////////////////
  ...
  if($i%2==0)
  {
   $template->assign_block_vars('job_category1', array(
                                // 'category_name'=>'<a class="d-block py-1" href="'.$ide.'/'.encode_category($key1).'-jobs.html'.'"  title="'.tep_db_output($key1).'"">'.tep_db_output($key1).$jobs,'</a>',
                                'category_name'=> tep_draw_form('search', FILENAME_JOB_SEARCH,($edit?'sID='.$save_search_id:''),'post','class="form-signin"')
                                .tep_draw_hidden_field('action','search')
                                .tep_draw_hidden_field('job_category[]',$ide)
                                .tep_draw_hidden_field('search_by_text',$key1).
                                '<button type="submit" class="d-block py-1 btn btn-font">'.tep_db_output($key1).$jobs.'</button></form>',
                               ));
  }
  else
  {
   $template->assign_block_vars('job_category2', array(
                                // 'category_name'=>'<a class="d-block py-1" href="'.$ide.'/'.encode_category($key1).'-jobs.html'.'"  title="'.tep_db_output($key1).'""   >'.tep_db_output($key1).$jobs,'</a>'
                                'category_name'=> tep_draw_form('search', FILENAME_JOB_SEARCH,($edit?'sID='.$save_search_id:''),'post','class="form-signin" type="hidden"')
                                .tep_draw_hidden_field('action','search').tep_draw_hidden_field('job_category[]',$ide).
                                '<button type="submit" class="d-block py-1 btn btn-font">'.tep_db_output($key1).$jobs.'</button></form>',
                              ));
  }
  $i++;
...
else
{
 $template->assign_vars(array( 'hidden_fields' => $hidden_fields,
  ...
  //'INFO_TEXT_APPLY_BUTTON' => (($x>0)?(check_login("jobseeker")?tep_image_button(PATH_TO_BUTTON.'button_apply_selectedjob.gif', IMAGE_APPLY,'onclick="ckeck_application(\'\',\''.HOST_NAME.'\');" style="cursor:pointer;"'):tep_image_button(PATH_TO_BUTTON.'button_registered_user.gif', IMAGE_APPLY,'onclick="ckeck_application(\'\',\''.HOST_NAME.'\');" style="cursor:pointer;"').' '.tep_image_button(PATH_TO_BUTTON.'button_new_user.gif', IMAGE_APPLY,'onclick="ckeck_application(\'new\',\''.HOST_NAME.'\');" style="cursor:pointer;"')):''),
   ...
   'INFO_TEXT_APPLY_BUTTON' => (($x>0)?(check_login("jobseeker")?'<a class="btn btn-primary" onclick="ckeck_application(\'\');" role="button">Apply to Selected Jobs</a>':'<a class="btn btn-primary" onclick="ckeck_application(\'\');" role="button">Registered User</a><a class="ml-2 btn btn-success m-btn-success" onclick="ckeck_application(\'new\');" role="button">New User</a>'):''),
